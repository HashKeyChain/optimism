...
//! **version-resolver-gated** dispatch path (`B20Spec::Beryl` -> `StablecoinVersion::V1`)
...
use b20_precompile_storage::{BasePrecompileError, HashMapStorageProvider, StorageCtx};
use hsk_b20_precompiles::{
    B20_MAX_SUPPLY_CAP, B20PolicyType, B20Spec, B20StablecoinInit, B20StablecoinStorage,
    B20StablecoinToken, B20TokenRole, FakePolicyAccounting, IB20, IB20Stablecoin,
...
fn op(
    ...
    StorageCtx::enter(storage, |ctx| {
        let version = StablecoinVersions::from_spec(B20Spec::Beryl).expect("Beryl activates V1");
...
fn resolver_maps_forks_to_versions() {
    assert_eq!(StablecoinVersions::from_spec(B20Spec::Disabled), None);
    assert_eq!(StablecoinVersions::from_spec(B20Spec::Beryl), Some(StablecoinVersion::V1));
}
...
fn dispatch_rejects_nonzero_value() {
    ...
    let out = StorageCtx::enter(&mut s, |ctx| {
        ...
        .dispatch_with_observer(ctx, &calldata, B20Spec::Beryl, NoopPrecompileCallObserver)
    })
...
fn golden_dispatch_no_observer_wrapper_reverts_uninitialized() {
    ...
    let out = StorageCtx::enter(&mut s, |ctx| {
        ...
        .dispatch(ctx, &calldata, B20Spec::Beryl)
    })
    .expect("dispatch must not fatally error");
    assert!(out.is_revert());
}
...
fn golden_inner_reverts_before_beryl() {
    ...
    let err = StorageCtx::enter(&mut s, |ctx| {
        ...
        StablecoinVersions::from_spec(B20Spec::Disabled).map_or_else(
