...
use hsk_b20_precompiles::{
    ...
    B20AssetStorage, B20FactoryStorage, B20Spec, B20StablecoinStorage, B20TokenRole, B20Variant,
...
fn resolver_maps_forks_to_versions() {
    assert_eq!(FactoryVersions::from_spec(B20Spec::Disabled), None);
    assert_eq!(FactoryVersions::from_spec(B20Spec::Beryl), Some(FactoryVersion::V1));
}
...
fn dispatch_reverts_before_beryl() {
    ...
    let (rev, bytes) = StorageCtx::enter(&mut s, |ctx| {
        B20FactoryStorage::new(ctx).dispatch(ctx, &calldata, B20Spec::Disabled)
    })
    .map(|out| (out.is_revert(), out.bytes))
    .expect("dispatch must not fatally error");
    assert!(rev);
