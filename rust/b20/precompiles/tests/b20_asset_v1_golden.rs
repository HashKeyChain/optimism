...
//! **version-resolver-gated** dispatch path (`B20Spec::Beryl` -> `AssetVersion::V1`) against
...
use hsk_b20_precompiles::{
    ...
    B20AssetStorage, B20AssetToken, B20PolicyType, B20Spec, B20TokenRole, FakePolicyAccounting,
    IB20, IB20Asset, NoopPrecompileCallObserver, PolicyVersion, TokenAccounting,
};
...
fn op(
    ...
    StorageCtx::enter(storage, |ctx| {
        let version = AssetVersions::from_spec(B20Spec::Beryl).expect("Beryl activates V1");
...
fn resolver_maps_forks_to_versions() {
    assert_eq!(AssetVersions::from_spec(B20Spec::Disabled), None);
    assert_eq!(AssetVersions::from_spec(B20Spec::Beryl), Some(AssetVersion::V1));
}
...
fn golden_inner_reverts_before_beryl() {
    ...
    let err = StorageCtx::enter(&mut s, |ctx| {
        ...
        AssetVersions::from_spec(B20Spec::Disabled).map_or_else(
