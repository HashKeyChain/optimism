        ...
        ::alloy_evm::precompiles::DynPrecompile::new_stateful(
            ...
            move |input| {
                if !input.is_direct_call() {
                    return ::b20_precompile_storage::BasePrecompileError::revert(
                        ::b20_precompile_storage::DelegateCallNotAllowed {},
                    )
                    .into_precompile_result(0, 0);
                }

                let $calldata: ::alloy_primitives::Bytes = input.data.to_vec().into();
                let mut provider = ::b20_precompile_storage::EvmPrecompileStorageProvider::new(
                    input,
                    ::revm::context_interface::cfg::GasParams::default(),
                );

                ::b20_precompile_storage::StorageCtx::enter(&mut provider, |$ctx| $impl)
            },
        )
    }};
    ($id:expr, |$input:ident, $ctx:ident, $calldata:ident| $impl:expr $(,)?) => {{
        ::alloy_evm::precompiles::DynPrecompile::new_stateful(
            ...
            move |$input| {
                ...
                let $calldata: ::alloy_primitives::Bytes = $input.data.to_vec().into();
                let mut provider =
                    ::b20_precompile_storage::EvmPrecompileStorageProvider::new($input);
...
macro_rules! decode_precompile_call {
    ($calldata:expr, $call_ty:ty $(,)?) => {{
        ...
        let selector = match calldata.get(..4) {
            ...
            None => {
                return Err(
                    ::b20_precompile_storage::BasePrecompileError::UnknownFunctionSelector(
                        [0u8; 4],
                    ),
                );
            }
        ...
        match <$call_ty as ::alloy_sol_types::SolInterface>::abi_decode_validate(calldata) {
            ...
            {
                return Err(::b20_precompile_storage::BasePrecompileError::AbiDecodeFailed {
                    selector,
...
mod tests {
    ...
    use b20_precompile_storage::{BasePrecompileError, Result};
