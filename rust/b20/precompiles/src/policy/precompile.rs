...
impl PolicyRegistryPrecompile {
    ...
    /// `upgrade`.
    pub fn install(precompiles: &mut PrecompilesMap, upgrade: B20Spec) {
    ...
    pub fn install_with_observer<O>(precompiles: &mut PrecompilesMap, upgrade: B20Spec, observer: O)
    where
    ...
    /// version active at `upgrade`.
    pub fn precompile_with_observer<O>(upgrade: B20Spec, observer: O) -> DynPrecompile
    where
        O: PrecompileCallObserver,
    {
