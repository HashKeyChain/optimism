...
use b20_precompile_storage::{BasePrecompileError, Result, StorageCtx};
...
mod tests {
    ...
    fn dispatch_factory_success(ctx: StorageCtx<'_>, call: impl SolCall) -> Bytes {
        ...
        let output = factory.dispatch(ctx, &call.abi_encode(), B20Spec::Beryl).unwrap();
    ...
    fn dispatch_b20_success(ctx: StorageCtx<'_>, token_addr: Address, call: impl SolCall) -> Bytes {
        ...
        let output = token.dispatch(ctx, &call.abi_encode(), B20Spec::Beryl).unwrap();
    ...
    fn dispatch_rejects_call_with_nonzero_value() {
        ...
        let out = StorageCtx::enter(&mut storage, |ctx| {
            B20FactoryStorage::new(ctx).dispatch(ctx, &calldata, B20Spec::Beryl)
        })
    ...
    fn test_uninitialized_prefix_token_reverts() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            let mut token = token_at(token_addr, ctx);
            let result =
                token.dispatch(ctx, &IB20::nameCall {}.abi_encode(), B20Spec::Beryl).unwrap();
