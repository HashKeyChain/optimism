...
use b20_precompile_storage::{BasePrecompileError, ContractStorage, Result};
...
mod tests {
    ...
    use b20_precompile_storage::{Handler, HashMapStorageProvider, StorageCtx};
    ...
    fn test_create_token_deploys_ef_stub() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            let token = factory.create_b20(caller, b20_call(salt), B20Spec::Beryl).unwrap();
    ...
    fn test_create_token_stores_metadata_and_decimals() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            let mut factory = B20FactoryStorage::new(ctx);
            let token_addr = factory.create_b20(caller, call, B20Spec::Beryl).unwrap();
    ...
    fn test_create_token_init_calls_use_factory_caller_and_restore_creator() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            let mut factory = B20FactoryStorage::new(ctx);
            let token_addr = factory.create_b20(creator, call, B20Spec::Beryl).unwrap();
            let token = B20AssetStorage::from_address(token_addr, ctx);
    ...
    fn test_create_token_reverts_if_salt_reused() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            let mut factory = B20FactoryStorage::new(ctx);
            factory.create_b20(caller, b20_call(salt), B20Spec::Beryl).unwrap();
            let result = factory.create_b20(caller, b20_call(salt), B20Spec::Beryl);
    ...
    fn test_create_token_reverts_for_invalid_version_and_variant() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            assert!(factory.create_b20(caller, bad_version, B20Spec::Beryl).is_err());
            ...
            assert!(factory.create_b20(caller, bad_variant, B20Spec::Beryl).is_err());
        });
    }
    ...
    fn test_create_asset_token_grants_default_admin_role() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            let mut factory = B20FactoryStorage::new(ctx);
            let token_addr = factory.create_b20(caller, call_no_admin, B20Spec::Beryl).unwrap();
