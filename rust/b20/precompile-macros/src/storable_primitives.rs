...
fn gen_storable_layout_impl(type_path: &TokenStream, byte_count: usize) -> TokenStream {
    quote! {
        impl ::b20_precompile_storage::StorableType for #type_path {
            const LAYOUT: ::b20_precompile_storage::Layout = ::b20_precompile_storage::Layout::Bytes(#byte_count);
...
fn gen_storage_key_impl(type_path: &TokenStream, strategy: &StorageKeyStrategy) -> TokenStream {
    ...
    quote! {
        impl ::b20_precompile_storage::StorageKey for #type_path {
            #[inline]
...
fn gen_to_word_impl(type_path: &TokenStream, strategy: &StorableConversionStrategy) -> TokenStream {
    match strategy {
        StorableConversionStrategy::UnsignedRust => quote! {
            impl ::b20_precompile_storage::FromWord for #type_path {
                #[inline]
                fn to_word(&self) -> ::alloy_primitives::U256 {
                    ::alloy_primitives::U256::from(*self)
                }
                #[inline]
                fn from_word(word: ::alloy_primitives::U256) -> ::b20_precompile_storage::Result<Self> {
                    word.try_into().map_err(|_| ::b20_precompile_storage::BasePrecompileError::under_overflow())
                }
            }
        },
            ...
            impl ::b20_precompile_storage::FromWord for #type_path {
                ...
                fn from_word(word: ::alloy_primitives::U256) -> ::b20_precompile_storage::Result<Self> {
                    if word > ::alloy_primitives::U256::from(::alloy_primitives::aliases::#ty::MAX) {
                        return Err(::b20_precompile_storage::BasePrecompileError::under_overflow());
                    }
                    Ok(word.to::<Self>())
                }
            }
            ...
            impl ::b20_precompile_storage::FromWord for #type_path {
                ...
                fn from_word(word: ::alloy_primitives::U256) -> ::b20_precompile_storage::Result<Self> {
                    let unsigned: #unsigned_type = word.try_into()
                        .map_err(|_| ::b20_precompile_storage::BasePrecompileError::under_overflow())?;
                    Ok(unsigned as Self)
                }
            }
        },
...
fn gen_complete_impl_set(config: &TypeConfig) -> TokenStream {
    ...
    let full_word_storable_impl = if config.byte_count < 32 {
        quote! {
            impl ::b20_precompile_storage::sealed::OnlyPrimitives for #type_path {}
            impl ::b20_precompile_storage::Packable for #type_path {}
        }
    } else {
        quote! {
            impl ::b20_precompile_storage::sealed::OnlyPrimitives for #type_path {}
            impl ::b20_precompile_storage::Storable for #type_path {
                #[inline]
                fn load<S: ::b20_precompile_storage::StorageOps>(
                    ...
                    storage.load(slot).and_then(<Self as ::b20_precompile_storage::FromWord>::from_word)
                }
                #[inline]
                fn store<S: ::b20_precompile_storage::StorageOps>(
                    ...
                    storage.store(slot, <Self as ::b20_precompile_storage::FromWord>::to_word(self))
                }
            }
        }
    };
...
fn gen_array_impl(config: &ArrayConfig) -> TokenStream {
    ...
    let slot_count_expr = if *elem_is_packable {
        quote! { ::b20_precompile_storage::calc_packed_slot_count(#array_size, #elem_byte_count) }
    } else {
        quote! { #array_size }
    };
    ...
    quote! {
        impl ::b20_precompile_storage::StorableType for [#elem_type; #array_size] {
            const LAYOUT: ::b20_precompile_storage::Layout = ::b20_precompile_storage::Layout::Slots(#slot_count_expr);
            type Handler<'a> = ::b20_precompile_storage::ArrayHandler<'a, #elem_type, #array_size>;
            ...
            fn handle<'a>(
                ...
                debug_assert_eq!(ctx, ::b20_precompile_storage::LayoutCtx::FULL, "Arrays cannot be packed");
                Self::Handler::new(slot, address, storage)
            }
        }

        impl ::b20_precompile_storage::Storable for [#elem_type; #array_size] {
            #[inline]
            fn load<S: ::b20_precompile_storage::StorageOps>(storage: &S, slot: ::alloy_primitives::U256, ctx: ::b20_precompile_storage::LayoutCtx) -> ::b20_precompile_storage::Result<Self> {
                debug_assert_eq!(ctx, ::b20_precompile_storage::LayoutCtx::FULL, "Arrays can only be loaded with LayoutCtx::FULL");
                use ::b20_precompile_storage::{calc_element_slot, calc_element_offset, extract_from_word};
                let base_slot = slot;
                #load_impl
            }

            #[inline]
            fn store<S: ::b20_precompile_storage::StorageOps>(&self, storage: &mut S, slot: ::alloy_primitives::U256, ctx: ::b20_precompile_storage::LayoutCtx) -> ::b20_precompile_storage::Result<()> {
                debug_assert_eq!(ctx, ::b20_precompile_storage::LayoutCtx::FULL, "Arrays can only be stored with LayoutCtx::FULL");
                use ::b20_precompile_storage::{calc_element_slot, calc_element_offset, insert_into_word};
                let base_slot = slot;
                #store_impl
            }
        }
...
fn gen_packed_array_store(array_size: &usize, elem_byte_count: &usize) -> TokenStream {
    quote! {
        let slot_count = ::b20_precompile_storage::calc_packed_slot_count(#array_size, #elem_byte_count);
...
fn gen_unpacked_array_load(array_size: &usize) -> TokenStream {
    quote! {
        ...
        for i in 0..#array_size {
            ...
            result[i] = ::b20_precompile_storage::Storable::load(storage, elem_slot, ::b20_precompile_storage::LayoutCtx::FULL)?;
        }
        Ok(result)
    }
}
...
fn gen_unpacked_array_store() -> TokenStream {
    quote! {
        for (i, elem) in self.iter().enumerate() {
            let elem_slot = base_slot + ::alloy_primitives::U256::from(i);
            ::b20_precompile_storage::Storable::store(elem, storage, elem_slot, ::b20_precompile_storage::LayoutCtx::FULL)?;
        }
        Ok(())
    }
}
...
fn gen_struct_array_impl(struct_type: &TokenStream, array_size: usize) -> TokenStream {
    ...
    quote! {
        mod #mod_ident {
            use super::*;
            pub const ELEM_SLOTS: usize = <#struct_type as ::b20_precompile_storage::StorableType>::SLOTS;
            pub const ARRAY_LEN: usize = #array_size;
            pub const SLOT_COUNT: usize = ARRAY_LEN * ELEM_SLOTS;
        }

        impl ::b20_precompile_storage::StorableType for [#struct_type; #array_size] {
        ...
        impl ::b20_precompile_storage::Storable for [#struct_type; #array_size] {
            #[inline]
            fn load<S: ::b20_precompile_storage::StorageOps>(storage: &S, slot: ::alloy_primitives::U256, ctx: ::b20_precompile_storage::LayoutCtx) -> ::b20_precompile_storage::Result<Self> {
                debug_assert_eq!(ctx, ::b20_precompile_storage::LayoutCtx::FULL, "Struct arrays can only be loaded with LayoutCtx::FULL");
                let base_slot = slot;
                #load_impl
            }
            ...
            #[inline]
            fn store<S: ::b20_precompile_storage::StorageOps>(&self, storage: &mut S, slot: ::alloy_primitives::U256, ctx: ::b20_precompile_storage::LayoutCtx) -> ::b20_precompile_storage::Result<()> {
                debug_assert_eq!(ctx, ::b20_precompile_storage::LayoutCtx::FULL, "Struct arrays can only be stored with LayoutCtx::FULL");
                let base_slot = slot;
                #store_impl
            }
        }
        ...
        for i in 0..#array_size {
            let elem_slot = base_slot.checked_add(
                ::alloy_primitives::U256::from(i).checked_mul(
                    ::alloy_primitives::U256::from(<#struct_type as ::b20_precompile_storage::StorableType>::SLOTS)
                ).ok_or(::b20_precompile_storage::BasePrecompileError::SlotOverflow)?
            ).ok_or(::b20_precompile_storage::BasePrecompileError::SlotOverflow)?;
            result[i] = <#struct_type as ::b20_precompile_storage::Storable>::load(storage, elem_slot, ::b20_precompile_storage::LayoutCtx::FULL)?;
        }
        Ok(result)
    }
}
...
fn gen_struct_array_store(struct_type: &TokenStream) -> TokenStream {
    quote! {
        for (i, elem) in self.iter().enumerate() {
            ...
            <#struct_type as ::b20_precompile_storage::Storable>::store(elem, storage, elem_slot, ::b20_precompile_storage::LayoutCtx::FULL)?;
        }
        Ok(())
    }
}
