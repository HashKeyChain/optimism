...
fn derive_struct_impl(input: &DeriveInput, data_struct: &DataStruct) -> syn::Result<TokenStream> {
    ...
    let is_dynamic = if direct_tys.is_empty() {
        ...
        quote! {
            #(
                <#direct_tys as ::b20_precompile_storage::StorableType>::IS_DYNAMIC
            )||*
        }
    };
    ...
    let expanded = quote! {
        ...
        impl #impl_generics ::b20_precompile_storage::StorableType for #strukt #ty_generics #where_clause {
            const LAYOUT: ::b20_precompile_storage::Layout = ::b20_precompile_storage::Layout::Slots(#mod_ident::SLOT_COUNT);
            #namespace_consts
            ...
            fn handle<'a>(
                ...
                _ctx: ::b20_precompile_storage::LayoutCtx,
                address: ::alloy_primitives::Address,
                storage: ::b20_precompile_storage::StorageCtx<'a>,
            ) -> Self::Handler<'a> {
                #handler_name::new(slot, address, storage)
            }
        }

        impl #impl_generics ::b20_precompile_storage::Storable for #strukt #ty_generics #where_clause {
            fn load<S: ::b20_precompile_storage::StorageOps>(
                storage: &S,
                base_slot: ::alloy_primitives::U256,
                ctx: ::b20_precompile_storage::LayoutCtx
            ) -> ::b20_precompile_storage::Result<Self> {
                use ::b20_precompile_storage::Storable;
                debug_assert_eq!(ctx, ::b20_precompile_storage::LayoutCtx::FULL, "Struct types can only be loaded with LayoutCtx::FULL");
            ...
            fn store<S: ::b20_precompile_storage::StorageOps>(
                &self,
                storage: &mut S,
                base_slot: ::alloy_primitives::U256,
                ctx: ::b20_precompile_storage::LayoutCtx
            ) -> ::b20_precompile_storage::Result<()> {
                use ::b20_precompile_storage::Storable;
                debug_assert_eq!(ctx, ::b20_precompile_storage::LayoutCtx::FULL, "Struct types can only be stored with LayoutCtx::FULL");
            ...
            fn delete<S: ::b20_precompile_storage::StorageOps>(
                storage: &mut S,
                base_slot: ::alloy_primitives::U256,
                ctx: ::b20_precompile_storage::LayoutCtx
            ) -> ::b20_precompile_storage::Result<()> {
                use ::b20_precompile_storage::Storable;
                debug_assert_eq!(ctx, ::b20_precompile_storage::LayoutCtx::FULL, "Struct types can only be deleted with LayoutCtx::FULL");
...
fn derive_unit_enum_impl(input: &DeriveInput, data_enum: &DataEnum) -> syn::Result<TokenStream> {
    ...
    Ok(quote! {
        impl #impl_generics ::b20_precompile_storage::StorableType for #enum_name #ty_generics #where_clause {
            const LAYOUT: ::b20_precompile_storage::Layout = ::b20_precompile_storage::Layout::Bytes(1);
            type Handler<'a> = ::b20_precompile_storage::Slot<'a, Self>;
            ...
            fn handle<'a>(
                ...
                ::b20_precompile_storage::Slot::new_with_ctx(slot, ctx, address, storage)
            }
        }

        impl #impl_generics ::b20_precompile_storage::Storable for #enum_name #ty_generics #where_clause {
            #[inline]
            fn load<S: ::b20_precompile_storage::StorageOps>(
                storage: &S,
                slot: ::alloy_primitives::U256,
                ...
                let value = <u8 as ::b20_precompile_storage::Storable>::load(storage, slot, ctx)?;
                match value {
                    #(discriminant if discriminant == Self::#variant_names as u8 => Ok(Self::#variant_names),)*
                    _ => Err(::b20_precompile_storage::BasePrecompileError::enum_conversion_error()),
                }
            }
            ...
            fn store<S: ::b20_precompile_storage::StorageOps>(
                ...
                <u8 as ::b20_precompile_storage::Storable>::store(&value, storage, slot, ctx)
            }
        }
    })
}
    ...
    quote! {
        pub mod #mod_ident {
            ...
            pub const SLOT_COUNT: usize = (#last_slot_const.saturating_add(
                ::alloy_primitives::U256::from_limbs([<#last_type as ::b20_precompile_storage::StorableType>::SLOTS as u64, 0, 0, 0])
            )).as_limbs()[0] as usize;
        }
    }
}
...
fn gen_handler_struct(
    ...
    Ok(quote! {
        ...
        impl<'a> #handler_name<'a> {
            ...
            #[inline]
            fn as_slot(&self) -> ::b20_precompile_storage::Slot<'a, #struct_name> {
                ::b20_precompile_storage::Slot::<#struct_name>::new(
                    self.base_slot,
                    self.address,
                    self.storage,
                )
        ...
        impl ::b20_precompile_storage::Handler<#struct_name> for #handler_name<'_> {
            #[inline]
            fn read(&self) -> ::b20_precompile_storage::Result<#struct_name> {
                self.as_slot().read()
            }
            #[inline]
            fn write(&mut self, value: #struct_name) -> ::b20_precompile_storage::Result<()> {
                self.as_slot().write(value)
            }
            #[inline]
            fn delete(&mut self) -> ::b20_precompile_storage::Result<()> {
                self.as_slot().delete()
            }
            #[inline]
            fn t_read(&self) -> ::b20_precompile_storage::Result<#struct_name> {
                self.as_slot().t_read()
            }
            #[inline]
            fn t_write(&mut self, value: #struct_name) -> ::b20_precompile_storage::Result<()> {
                self.as_slot().t_write(value)
            }
            #[inline]
            fn t_delete(&mut self) -> ::b20_precompile_storage::Result<()> {
...
fn gen_access_method(field: &AccessField, spec: &AccessSpec) -> syn::Result<TokenStream> {
    ...
    Ok(quote! {
        #[doc = #doc]
        pub fn #name(&self, #(#args),*) -> ::b20_precompile_storage::Result<#value_ty> {
            ::b20_precompile_storage::Handler::read(#target)
        }
    })
}
...
fn gen_mutate_method(field: &AccessField, spec: &AccessSpec) -> syn::Result<TokenStream> {
    ...
    Ok(quote! {
        ...
        pub fn #name(
            ...
            ::b20_precompile_storage::Handler::write(#target, #value)
        }
    })
}
...
fn gen_load_impl(fields: &[(&Ident, &Type)], packing: &Ident) -> TokenStream {
    ...
    let field_loads = fields.iter().enumerate().map(|(idx, (name, ty))| {
        ...
        let packed_ctx = quote! { ::b20_precompile_storage::LayoutCtx::packed(#packing::#loc_const.offset_bytes) };

        prev_slot_ref.map_or_else(
            || quote! {
                let #name = if <#ty as ::b20_precompile_storage::StorableType>::IS_PACKABLE {
                    cached_slot = storage.load(#slot_addr)?;
                    let packed = ::b20_precompile_storage::PackedSlot(cached_slot);
                    <#ty as ::b20_precompile_storage::Storable>::load(&packed, ::alloy_primitives::U256::ZERO, #packed_ctx)?
                } else {
                    <#ty as ::b20_precompile_storage::Storable>::load(storage, #slot_addr, ::b20_precompile_storage::LayoutCtx::FULL)?
                };
            },
            |prev_slot_ref| quote! {
                let #name = {
                    ...
                    if <#ty as ::b20_precompile_storage::StorableType>::IS_PACKABLE && curr_offset == prev_offset {
                        let packed = ::b20_precompile_storage::PackedSlot(cached_slot);
                        <#ty as ::b20_precompile_storage::Storable>::load(&packed, ::alloy_primitives::U256::ZERO, #packed_ctx)?
                    } else if <#ty as ::b20_precompile_storage::StorableType>::IS_PACKABLE {
                        cached_slot = storage.load(#slot_addr)?;
                        let packed = ::b20_precompile_storage::PackedSlot(cached_slot);
                        <#ty as ::b20_precompile_storage::Storable>::load(&packed, ::alloy_primitives::U256::ZERO, #packed_ctx)?
                    } else {
...
fn gen_store_impl(fields: &[(&Ident, &Type)], packing: &Ident) -> TokenStream {
    ...
    let field_stores = fields.iter().enumerate().map(|(idx, (name, ty))| {
            ...
            (Some(next_slot), Some(next_ty)) => {
                quote! {
                    #packing::#loc_const.offset_slots != #next_slot
                        || !<#next_ty as ::b20_precompile_storage::StorableType>::IS_PACKABLE
                }
            }
            _ => quote! { true },
        };
        ...
        prev_slot_ref.map_or_else(
            || quote! {{
                if <#ty as ::b20_precompile_storage::StorableType>::IS_PACKABLE {
                    ...
                    let mut packed = ::b20_precompile_storage::PackedSlot(pending_val);
                    <#ty as ::b20_precompile_storage::Storable>::store(&self.#name, &mut packed, ::alloy_primitives::U256::ZERO, #packed_ctx)?;
                    ...
                    <#ty as ::b20_precompile_storage::Storable>::store(&self.#name, storage, #slot_addr, ::b20_precompile_storage::LayoutCtx::FULL)?;
                }
            }},
            |prev_slot_ref| quote! {{
                let curr_offset = #packing::#loc_const.offset_slots;
...
fn gen_delete_impl(fields: &[(&Ident, &Type)], packing: &Ident) -> TokenStream {
    ...
    let dynamic_deletes = fields.iter().map(|(name, ty)| {
        let loc_const = PackingConstants::new(name).location();
        quote! {
            if <#ty as ::b20_precompile_storage::StorableType>::IS_DYNAMIC {
                <#ty as ::b20_precompile_storage::Storable>::delete(
                    storage,
                    base_slot + ::alloy_primitives::U256::from(#packing::#loc_const.offset_slots),
                    ::b20_precompile_storage::LayoutCtx::FULL
                )?;
            }
        }
    });
    ...
    let is_static_slot = fields.iter().map(|(name, ty)| {
        let loc_const = PackingConstants::new(name).location();
        quote! {
            ((#packing::#loc_const.offset_slots..#packing::#loc_const.offset_slots + <#ty as ::b20_precompile_storage::StorableType>::SLOTS)
                .contains(&slot_offset) &&
             !<#ty as ::b20_precompile_storage::StorableType>::IS_DYNAMIC)
        }
    });
