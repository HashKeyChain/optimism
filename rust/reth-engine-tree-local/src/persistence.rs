    ...
    N: ProviderNodeTypes,
{
    ...
    fn on_save_blocks(
        ...
        if let Some(last) = last_block {
            ...
            let t_save_blocks = t_save_start.elapsed();
            ...
            let mut prune_elapsed = std::time::Duration::ZERO;
            if self.pruner.is_pruning_needed(last.number) {
                ...
                prune_elapsed = prune_start.elapsed();
                self.metrics.prune_before_duration_seconds.record(prune_elapsed);
            }
            ...
            info!(
                ...
                first = ?first_block,
                last = ?last_block,
                save_blocks_ms = t_save_blocks.as_millis(),
                prune_ms = prune_elapsed.as_millis(),
                commit_ms = t_commit.as_millis(),
                total_ms = start_time.elapsed().as_millis(),
                "[PERF] persistence write"
            );
        }
