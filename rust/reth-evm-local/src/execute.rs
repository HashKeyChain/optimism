    ...
    N: NodePrimitives,
{
    ...
    fn finish(
        ...
        tracing::info!(
            ...
            evm_finish_ms = t_evm_finish.as_millis(),
            merge_ms = (t_merge - t_evm_finish).as_millis(),
            hashed_post_state_ms = (t_hashed - t_merge).as_millis(),
            state_root_ms = (t_root - t_hashed).as_millis(),
            assemble_ms = (t_total - t_root).as_millis(),
            finish_total_ms = t_total.as_millis(),
            "[PERF] builder.finish() breakdown"
        );
