...
//! against the real EVM-backed `H20StablecoinStorage` over `HashMapStorageProvider`, with an
...
//!
//! Because the per-op suite resolves the version via `StablecoinVersions::from_spec`,
...
fn seed(storage: &mut HashMapStorageProvider, f: impl FnOnce(&mut H20StablecoinStorage<'_>)) {
...
fn op(
    ...
    StorageCtx::enter(storage, |ctx| {
        ...
        H20StablecoinToken::with_storage_and_policy(
            H20StablecoinStorage::from_address(TOKEN, ctx),
            policy,
            PolicyVersion::V1,
        )
...
fn give_role(token: &mut H20StablecoinStorage<'_>, role: B256, who: Address) {
...
fn fund(token: &mut H20StablecoinStorage<'_>, who: Address, amount: U256) {
...
fn domain_separator(storage: &mut HashMapStorageProvider) -> B256 {
    StorageCtx::enter(storage, |ctx| {
        let token = H20StablecoinToken::with_storage_and_policy(
            H20StablecoinStorage::from_address(TOKEN, ctx),
...
fn resolver_maps_forks_to_versions() {
    assert_eq!(StablecoinVersions::from_spec(H20Spec::Disabled), None);
    assert_eq!(StablecoinVersions::from_spec(H20Spec::Beryl), Some(StablecoinVersion::V1));
}
...
fn golden_read_currency() {
    ...
    let out = op(
        ...
        ALICE,
        FakePolicyAccounting::new(),
        IH20Stablecoin::currencyCall {}.abi_encode(),
    )
    .unwrap();
...
fn gas(
    setup: impl FnOnce(&mut H20StablecoinStorage<'_>),
    caller: Address,
    policy: FakePolicyAccounting,
    calldata: Vec<u8>,
) -> (u64, u64, u64) {
...
/// reviewed.
#[allow(dead_code)]
fn v1_op_coverage_checklist(call: IH20::IH20Calls, ext: IH20Stablecoin::IH20StablecoinCalls) {
    use IH20::IH20Calls as C;
    use IH20Stablecoin::IH20StablecoinCalls as SC;
