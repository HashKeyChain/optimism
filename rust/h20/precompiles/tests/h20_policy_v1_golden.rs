...
//! `H20Spec::Beryl` -> `PolicyVersion::V1`) over `HashMapStorageProvider`.
//! Each case asserts:
...
use hsk_h20_precompiles::{
    ActivationAdminConfig, ActivationFeature, ActivationRegistryStorage, H20Spec, IPolicyRegistry,
...
fn call_policy(
    ...
    StorageCtx::enter(storage, |ctx| {
        PolicyRegistryStorage::new(ctx).dispatch(ctx, &calldata, H20Spec::Beryl)
    })
...
fn resolver_maps_forks_to_versions() {
    assert_eq!(PolicyVersions::from_spec(H20Spec::Disabled), None);
    assert_eq!(PolicyVersions::from_spec(H20Spec::Beryl), Some(PolicyVersion::V1));
}
...
fn dispatch_reverts_before_beryl() {
    ...
    let (rev, bytes) = StorageCtx::enter(&mut s, |ctx| {
        PolicyRegistryStorage::new(ctx).dispatch(ctx, &calldata, H20Spec::Disabled)
    })
    .map(|out| (out.is_revert(), out.bytes))
    .expect("dispatch must not fatally error");
    assert!(rev);
