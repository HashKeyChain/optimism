...
//! through the real `H20FactoryStorage` entry (version-resolver-gated `dispatch`) over
...
//!   3. emitted events (`H20Created` at the factory + role/init events at the token), and
...
/// A `createH20` call for the given variant/params.
fn create_call(
    ...
    IH20Factory::createH20Call { variant, salt, params, initCalls: init_calls }.abi_encode()
}
...
fn read_asset<R>(
    storage: &mut HashMapStorageProvider,
    token: Address,
    f: impl FnOnce(&H20AssetStorage<'_>) -> R,
) -> R {
...
fn read_stablecoin<R>(
    storage: &mut HashMapStorageProvider,
    token: Address,
    f: impl FnOnce(&H20StablecoinStorage<'_>) -> R,
) -> R {
    StorageCtx::enter(storage, |ctx| f(&H20StablecoinStorage::from_address(token, ctx)))
}
...
fn dispatch_reverts_before_beryl() {
    let mut s = fresh();
    let calldata = IH20Factory::isH20Call { token: asset_addr(CREATOR, SALT) }.abi_encode();
    s.set_caller(ALICE);
    let (rev, bytes) = StorageCtx::enter(&mut s, |ctx| {
        H20FactoryStorage::new(ctx).dispatch(ctx, &calldata, H20Spec::Disabled)
    })
    .map(|out| (out.is_revert(), out.bytes))
    .expect("dispatch must not fatally error");
    assert!(rev);
...
// ============================================================================
// getH20Address (deterministic address derivation)
// ============================================================================

#[test]
fn golden_get_h20_address() {
    let mut s = fresh();
    ...
    assert_eq!(
        bytes,
        Bytes::from(IH20Factory::getH20AddressCall::abi_encode_returns(&asset_addr(CREATOR, SALT)))
    );
    ...
    assert_eq!(
        bytes,
        Bytes::from(IH20Factory::getH20AddressCall::abi_encode_returns(&stablecoin_addr(
            CREATOR, SALT
        )))
    );
}
...
fn golden_is_h20() {
    ...
    let (rev, bytes) = call_factory(
        &mut s,
        ALICE,
        IH20Factory::isH20Call { token: asset_addr(CREATOR, SALT) }.abi_encode(),
    );
    assert!(!rev);
    assert_eq!(bytes, Bytes::from(IH20Factory::isH20Call::abi_encode_returns(&true)));

    // A random address does not.
    let (rev, bytes) = call_factory(
        &mut s,
        ALICE,
        IH20Factory::isH20Call { token: Address::repeat_byte(0x99) }.abi_encode(),
    );
    assert!(!rev);
    assert_eq!(bytes, Bytes::from(IH20Factory::isH20Call::abi_encode_returns(&false)));
}

#[test]
fn golden_is_h20_initialized() {
    let mut s = fresh();
    let token = asset_addr(CREATOR, SALT);
    // Prefix address but not yet created => not initialized.
    let (rev, bytes) =
        call_factory(&mut s, ALICE, IH20Factory::isH20InitializedCall { token }.abi_encode());
    assert!(!rev);
    assert_eq!(bytes, Bytes::from(IH20Factory::isH20InitializedCall::abi_encode_returns(&false)));
    ...
    let (rev, bytes) =
        call_factory(&mut s, ALICE, IH20Factory::isH20InitializedCall { token }.abi_encode());
    assert!(!rev);
    assert_eq!(bytes, Bytes::from(IH20Factory::isH20InitializedCall::abi_encode_returns(&true)));
}
...
fn golden_create_asset() {
    ...
    assert!(!rev);
    assert_eq!(bytes, Bytes::from(IH20Factory::createH20Call::abi_encode_returns(&token)));
    ...
    // H20Created emitted at the factory address.
    let created = s.get_events(factory());
    assert_eq!(created.last().unwrap().topics()[0], IH20Factory::H20Created::SIGNATURE_HASH);
...
fn golden_create_stablecoin() {
    ...
    let (rev, bytes) = call_factory(
        ...
        CREATOR,
        create_call(
            IH20Factory::H20Variant::STABLECOIN,
            SALT,
...
fn golden_create_stablecoin_with_init_calls() {
    let mut s = fresh();
    let token = stablecoin_addr(CREATOR, SALT);
    let mint = IH20::mintCall { to: ALICE, amount: u(500) }.abi_encode();
    let (rev, _) = call_factory(
        &mut s,
        CREATOR,
        create_call(
...
fn golden_create_with_init_calls() {
    ...
    let mint = IH20::mintCall { to: ALICE, amount: u(1_000) }.abi_encode();
    let (rev, _) = call_factory(
        &mut s,
        CREATOR,
        create_call(
...
fn golden_create_zero_admin_skips_grant() {
    ...
    assert!(!rev);
    read_asset(&mut s, token, |t| {
        assert!(!t.has_role(H20TokenRole::DefaultAdmin.id(), Address::ZERO).unwrap());
    });
...
fn golden_create_reverts_token_already_exists() {
    ...
    assert!(rev);
    assert_eq!(bytes, Bytes::from(IH20Factory::TokenAlreadyExists { token }.abi_encode()));
}

#[test]
fn golden_create_reverts_unsupported_version() {
    let mut s = fresh();
    let params: Bytes = IH20Factory::H20AssetCreateParams {
        version: 2,
        name: NAME.into(),
        symbol: SYMBOL.into(),
        initialAdmin: ADMIN,
    ...
    let (rev, bytes) = call_factory(
        &mut s,
        CREATOR,
        create_call(IH20Factory::H20Variant::ASSET, SALT, params, vec![]),
    );
    assert!(rev);
    assert_eq!(
        bytes,
        Bytes::from(
            IH20Factory::UnsupportedVersion { version: 2, variant: IH20Factory::H20Variant::ASSET }
                .abi_encode()
        )
    );
}
...
fn golden_create_reverts_invalid_decimals() {
    ...
    let (rev, bytes) = call_factory(
        &mut s,
        CREATOR,
        create_call(IH20Factory::H20Variant::ASSET, SALT, asset_params(ADMIN, 5), vec![]),
    );
    assert!(rev);
    assert_eq!(bytes, Bytes::from(IH20Factory::InvalidDecimals { decimals: 5 }.abi_encode()));
}
...
fn golden_create_reverts_nonzero_value() {
    ...
    .expect("dispatch must not fatally error");
    assert!(rev);
    assert_eq!(bytes, Bytes::from(IH20Factory::NonPayable {}.abi_encode()));
}
...
fn golden_create_reverts_init_call_failed() {
    ...
    assert!(rev);
    assert_eq!(bytes, Bytes::from(IH20Factory::InitCallFailed { index: U256::ZERO }.abi_encode()));
}
...
fn golden_create_reverts_missing_currency() {
    ...
    assert_eq!(
        bytes,
        Bytes::from(IH20Factory::MissingRequiredField { field: "currency".into() }.abi_encode())
    );
}
...
fn golden_create_reverts_invalid_currency() {
    ...
    assert_eq!(
        bytes,
        Bytes::from(IH20Factory::InvalidCurrency { code: "usd".into() }.abi_encode())
    );
}
...
fn golden_create_propagates_typed_init_call_revert() {
    ...
    let mut s = fresh();
    let mint_zero = IH20::mintCall { to: Address::ZERO, amount: u(1) }.abi_encode();
    let (rev, bytes) = call_factory(
        &mut s,
        CREATOR,
        create_call(
    ...
    assert!(rev);
    assert_eq!(bytes, Bytes::from(IH20::InvalidReceiver { receiver: Address::ZERO }.abi_encode()));
}
...
fn golden_create_reverts_malformed_params() {
    ...
    assert!(rev);
    // Malformed params surface as an AbiDecodeFailed carrying the createH20 selector.
    assert_eq!(&bytes[..4], IH20Factory::createH20Call::SELECTOR.as_slice());
}
...
fn golden_create_reverts_when_not_activated() {
    ...
    assert_eq!(
        bytes,
        Bytes::from(
            IActivationRegistry::FeatureNotActivated { feature: ActivationFeature::H20Asset.id() }
                .abi_encode()
        )
    );
}
...
fn golden_gas_footprints() {
    let actual: Vec<(&str, (u64, u64, u64))> = vec![
        ...
        (
            "is_h20",
            gas(ALICE, IH20Factory::isH20Call { token: asset_addr(CREATOR, SALT) }.abi_encode()),
        ),
        (
            "is_h20_initialized",
            gas(
                ALICE,
                IH20Factory::isH20InitializedCall { token: asset_addr(CREATOR, SALT) }.abi_encode(),
            ),
        ),
    ];
    ...
    let expected: &[(&str, (u64, u64, u64))] = &[
        ...
        ("get_h20_address", (0, 0, 1)),
        ("is_h20", (0, 0, 0)),
        ("is_h20_initialized", (0, 0, 0)),
    ];
...
fn v1_op_coverage_checklist(call: IH20Factory::IH20FactoryCalls) {
    ...
    match call {
        ...
        C::getH20Address(_) => covered(&[golden_get_h20_address]),
        C::isH20(_) => covered(&[golden_is_h20]),
        C::isH20Initialized(_) => covered(&[golden_is_h20_initialized]),
    }
}
