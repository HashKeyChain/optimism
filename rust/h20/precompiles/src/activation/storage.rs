...
pub struct ActivationAdminConfig {
    /// Static chain-config activation admin used by Beryl.
    pub fallback: Option<Address>,
}

impl ActivationAdminConfig {
    /// Creates the Beryl static activation admin configuration.
...
mod tests {
    ...
    const FEATURE: B256 = ActivationFeature::H20Asset.id();
    ...
    fn feature_id_constants_match_canonical_names() {
        ...
        assert_eq!(ActivationFeature::H20Stablecoin.id(), keccak256("base.b20_stablecoin"));
        assert_eq!(ActivationFeature::H20Asset.id(), keccak256("base.b20_asset"));
    }
