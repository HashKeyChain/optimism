...
mod tests {
    ...
    fn observe_brackets_result() {
        ...
        let result = observer.observe("precompile-h20-transfer", || 42);

        assert_eq!(result, 42);
        assert_eq!(
            observer.events(),
            [("start", "precompile-h20-transfer"), ("end", "precompile-h20-transfer"),]
        );
    }
    ...
    fn observe_ends_when_observed_work_panics() {
        ...
        let result = catch_unwind(AssertUnwindSafe(|| {
            observer.observe("precompile-h20-transfer", || panic!("observed panic"));
        }));
