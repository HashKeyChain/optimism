...
pub struct PrecompileCallMetric {
    /// Beryl precompile surface, such as `factory`, `activation`, `policy`, or `h20`.
...
impl PrecompileCallMetric {
    ...
    pub fn h20(
        ...
        Self::new("h20", method, Some(variant), input_bytes)
    }
...
impl BerylErrorKind {
    ...
    pub fn from_revert_bytes(bytes: &Bytes) -> Self {
        ...
        if BerylErrorClassifier::is_error_selector::<IH20::ContractPaused>(selector) {
            return Self::Paused;
        }
...
impl BerylMetricLabels {
    ...
    pub fn h20_method(label: &'static str) -> Cow<'static, str> {
        Cow::Borrowed(
            label
                .strip_prefix("precompile-h20-asset-")
                .or_else(|| label.strip_prefix("precompile-h20-stablecoin-"))
                .or_else(|| label.strip_prefix("precompile-h20-"))
                .unwrap_or(label),
        )
    }
    ...
    pub fn factory_method(calldata: &[u8]) -> Cow<'static, str> {
        match BerylSelector::selector(calldata) {
            Some(IH20Factory::createH20Call::SELECTOR) => Cow::Borrowed("createH20"),
            Some(IH20Factory::getH20AddressCall::SELECTOR) => Cow::Borrowed("getH20Address"),
            Some(IH20Factory::isH20Call::SELECTOR) => Cow::Borrowed("isH20"),
            Some(IH20Factory::isH20InitializedCall::SELECTOR) => Cow::Borrowed("isH20Initialized"),
            _ => Self::unknown(),
        }
    }
    ...
    pub fn h20_asset_method(calldata: &[u8]) -> Cow<'static, str> {
        ...
        if let Some(method) = IH20Asset::IH20AssetCalls::name_by_selector(selector) {
            return Cow::Owned(method.to_string());
        }
        if let Some(method) = IH20::IH20Calls::name_by_selector(selector) {
            return Cow::Owned(method.to_string());
        }
        Self::unknown()
    }
    ...
    pub fn h20_stablecoin_call(calldata: &[u8]) -> PrecompileCallMetric {
        PrecompileCallMetric::h20(
            "stablecoin",
            Self::h20_stablecoin_method(calldata),
            calldata.len(),
        )
    }
    ...
    pub fn h20_stablecoin_method(calldata: &[u8]) -> Cow<'static, str> {
        ...
        if let Some(method) = IH20Stablecoin::IH20StablecoinCalls::name_by_selector(selector) {
            return Cow::Owned(method.to_string());
        }
        if let Some(method) = IH20::IH20Calls::name_by_selector(selector) {
            return Cow::Owned(method.to_string());
...
mod tests {
    ...
    use crate::{
        ...
        CALLDATA_WORD_GAS, IH20, IH20Factory, IPolicyRegistry, NoopPrecompileCallObserver,
    };

    #[test]
    fn h20_method_labels_strip_precompile_prefixes() {
        assert_eq!(BerylMetricLabels::h20_method("precompile-h20-transfer"), "transfer");
        assert_eq!(BerylMetricLabels::h20_method("precompile-h20-stablecoin-currency"), "currency");
    }

    #[test]
    fn selector_method_labels_are_stable() {
        let factory = IH20Factory::getH20AddressCall {
            variant: IH20Factory::H20Variant::ASSET,
            sender: Address::ZERO,
            salt: B256::ZERO,
        }
        .abi_encode();
        assert_eq!(BerylMetricLabels::factory_method(&factory), "getH20Address");

        let h20 = IH20::transferCall { to: Address::ZERO, amount: U256::ZERO }.abi_encode();
        assert_eq!(BerylMetricLabels::h20_asset_method(&h20), "transfer");
    }
    ...
    fn revert_bytes_are_classified() {
        let unauthorized = IH20::Unauthorized {}.abi_encode().into();
    ...
    fn selector_extracts_revert_selector() {
        let bytes = IH20::Unauthorized {}.abi_encode();
        assert_eq!(
            BerylSelector::selector(&bytes),
            Some(<IH20::Unauthorized as SolError>::SELECTOR)
        );
    }
