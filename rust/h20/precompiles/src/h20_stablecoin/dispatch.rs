...
impl<S: StablecoinAccounting, A: PolicyAccounting> H20StablecoinToken<S, A> {
    /// ABI-dispatches `calldata` to the appropriate `IH20` handler for `upgrade`.
    pub fn dispatch(
        &mut self,
        ctx: StorageCtx<'_>,
        calldata: &[u8],
        ...
        O: PrecompileCallObserver,
    {
        let mut recorder = BerylCallRecorder::start(
            observer.clone(),
            BerylMetricLabels::h20_stablecoin_call(calldata),
        );
        ...
        observer.observe(label, || {
            let encoded: Bytes = match call {
                ...
                C::TRANSFER_SENDER_POLICY(_) => {
                    H20PolicyType::TransferSender.id().abi_encode().into()
                }
...
mod tests {
    ...
    fn make_token() -> H20StablecoinToken<InMemoryTokenAccounting, FakePolicyAccounting> {
    ...
    fn decimals_returns_fixed_six_regardless_of_storage() {
        ...
        let calldata = IH20::decimalsCall {}.abi_encode();
    ...
    fn dispatch_rejects_call_with_nonzero_value() {
        ...
        let calldata = IH20::balanceOfCall { account: Address::ZERO }.abi_encode();
        let mut storage = HashMapStorageProvider::new(1);
        storage.set_call_value(U256::from(1u64));

        let out = StorageCtx::enter(&mut storage, |ctx| {
            token.dispatch_with_observer(ctx, &calldata, H20Spec::Beryl, NoopPrecompileCallObserver)
        })
