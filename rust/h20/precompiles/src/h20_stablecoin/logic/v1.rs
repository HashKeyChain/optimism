...
use crate::{
    Eip712Domain, H20_MAX_SUPPLY_CAP, H20Guards, H20PausableFeature, H20PolicyType,
    H20StablecoinToken, H20TokenRole, IH20, PermitArgs, PolicyAccounting, Stablecoin,
...
impl StablecoinV1 {
    ...
    fn transfer_inner<S: StablecoinAccounting, A: PolicyAccounting>(
        &self,
        token: &mut H20StablecoinToken<S, A>,
        from: Address,
        to: Address,
        amount: U256,
        privileged: bool,
    ...
    /// the authorized [`grant_role`](Stablecoin::grant_role) path is not reachable. Bumps the
    /// `DefaultAdmin` member count and emits `RoleGranted`. Kept inherent to V1 (off the
    /// `Stablecoin` trait) so it stays frozen with this version and off `&dyn Stablecoin`.
...
impl<S: StablecoinAccounting, A: PolicyAccounting> Stablecoin<S, A> for StablecoinV1 {
    ...
    fn paused_features(
        &self,
        token: &H20StablecoinToken<S, A>,
    ) -> Result<Vec<IH20::PausableFeature>> {
    ...
    fn policy_id(&self, token: &H20StablecoinToken<S, A>, policy_scope: B256) -> Result<u64> {
        Self::ensure_supported_policy_type(policy_scope)?;
        token.accounting().policy_id(policy_scope)
    }

    fn domain_separator(&self, token: &H20StablecoinToken<S, A>, chain_id: u64) -> Result<B256> {
    ...
    fn currency(&self, token: &H20StablecoinToken<S, A>) -> Result<String> {
...
mod tests {
    ...
    use crate::{
        H20_MAX_SUPPLY_CAP, H20PolicyType, H20StablecoinToken, H20TokenRole, IH20, PackedPolicy,
    ...
    type Tok = H20StablecoinToken<FakeAccounting, FakePolicyAccounting>;
    ...
    fn transfer_unprivileged_enforces_transfer_policies() {
        ...
        tok.accounting_mut().set_policy_id(H20PolicyType::TransferSender.id(), POLICY).unwrap();
        tok.accounting_mut().set_policy_id(H20PolicyType::TransferReceiver.id(), POLICY).unwrap();
    ...
    fn pause_and_unpause_toggle_feature_bit() {
        ...
        assert!(!LOGIC.is_paused(&tok, IH20::PausableFeature::TRANSFER).unwrap());
    ...
    fn paused_features_reports_active_set() {
        ...
        LOGIC
            .pause(
                &mut tok,
                ADMIN,
                vec![IH20::PausableFeature::MINT, IH20::PausableFeature::BURN],
                true,
            )
            .unwrap();
        assert_eq!(LOGIC.paused_features(&tok).unwrap().len(), 2);
        assert!(LOGIC.is_paused(&tok, IH20::PausableFeature::BURN).unwrap());
    }
    ...
    fn update_supply_cap_sets_and_emits() {
        ...
        assert_eq!(last_event_sig(&tok), IH20::SupplyCapUpdated::SIGNATURE_HASH);
    }
    ...
    fn revoke_role_privileged_revokes_and_emits() {
        ...
        LOGIC.revoke_role(&mut tok, ADMIN, H20TokenRole::Mint.id(), ALICE, true).unwrap();
        assert!(!tok.accounting().has_role(H20TokenRole::Mint.id(), ALICE).unwrap());
        assert_eq!(last_event_sig(&tok), IH20::RoleRevoked::SIGNATURE_HASH);
    }
    ...
    fn update_policy_reverts_when_policy_missing() {
        let mut tok = token();
        let err = LOGIC
            .update_policy(&mut tok, ADMIN, H20PolicyType::TransferSender.id(), 99, true)
            .unwrap_err();
