...
pub trait Asset<S: AssetAccounting, A: PolicyAccounting> {
    ...
    fn is_initialized(&self, token: &H20AssetToken<S, A>) -> Result<bool> {
        token.accounting().is_initialized()
    }

    /// Returns the token name.
    fn name(&self, token: &H20AssetToken<S, A>) -> Result<String> {
        token.accounting().name()
    }

    /// Returns the token symbol.
    fn symbol(&self, token: &H20AssetToken<S, A>) -> Result<String> {
        token.accounting().symbol()
    }

    /// Returns the custom decimal precision configured for this asset token.
    fn decimals(&self, token: &H20AssetToken<S, A>) -> Result<u8> {
        AssetAccounting::decimals(token.accounting())
    }

    /// Returns the total token supply currently in circulation.
    fn total_supply(&self, token: &H20AssetToken<S, A>) -> Result<U256> {
        token.accounting().total_supply()
    }

    /// Returns the token balance of `account`.
    fn balance_of(&self, token: &H20AssetToken<S, A>, account: Address) -> Result<U256> {
