//! Version 1 of the asset H20 precompile logic, activated at Beryl.
...
/// First asset H20 implementation. Frozen as of its activation at Beryl.
...
impl AssetV1 {
    ...
    ///
    /// Asset-specific (not part of [`H20TokenRole`]); kept inherent to V1 so it stays frozen with
    ...
    fn transfer_inner<S: AssetAccounting, A: PolicyAccounting>(
        ...
        if !privileged {
            ...
            H20Guards::ensure_policy_type(token, H20PolicyType::TransferReceiver, to)?;
        }
    ...
    pub(crate) fn grant_role_unchecked<S: AssetAccounting, A: PolicyAccounting>(
        ...
        token
            .accounting_mut()
            .emit_event(IH20::RoleGranted { role, account, sender }.encode_log_data())
    }
    ...
    fn revoke_role_unchecked<S: AssetAccounting, A: PolicyAccounting>(
        ...
        token
            .accounting_mut()
            .emit_event(IH20::RoleRevoked { role, account, sender }.encode_log_data())
    }
    ...
    fn ensure_operator_role<S: AssetAccounting, A: PolicyAccounting>(
        ...
        if privileged { Ok(()) } else { H20Guards::ensure_role(token, caller, Self::OPERATOR_ROLE) }
    }
    ...
    fn ensure_metadata_role<S: AssetAccounting, A: PolicyAccounting>(
        ...
        if privileged {
            ...
            H20Guards::ensure_token_role(token, caller, H20TokenRole::Metadata)
        }
    }
}
...
impl<S: AssetAccounting, A: PolicyAccounting> Asset<S, A> for AssetV1 {
    ...
    fn transfer_from(
        ...
        if !privileged && caller != from {
            H20Guards::ensure_policy_type(token, H20PolicyType::TransferExecutor, caller)?;
        }
    ...
    fn approve(
        ...
        token
            .accounting_mut()
            .emit_event(IH20::Approval { owner: caller, spender, amount }.encode_log_data())
    }
    ...
    fn emit_memo(
        ...
        token.accounting_mut().emit_event(IH20::Memo { caller, memo }.encode_log_data())
    }
    ...
    fn mint(
        ...
        H20Guards::ensure_not_paused(token, IH20::PausableFeature::MINT)?;
        if !privileged {
    ...
    fn burn(&self, token: &mut H20AssetToken<S, A>, caller: Address, amount: U256) -> Result<()> {
        // Self-burn: `from == caller`, never factory-privileged.
        H20Guards::ensure_not_paused(token, IH20::PausableFeature::BURN)?;
        H20Guards::ensure_token_role(token, caller, H20TokenRole::Burn)?;
    ...
    fn burn_blocked(
        ...
        H20Guards::ensure_not_paused(token, IH20::PausableFeature::BURN)?;
        if !privileged {
            H20Guards::ensure_token_role(token, caller, H20TokenRole::BurnBlocked)?;
        }
        H20Guards::ensure_blocked(token, from)?;
    ...
    fn update_supply_cap(
        ...
        if !privileged {
            H20Guards::ensure_token_role(token, caller, H20TokenRole::DefaultAdmin)?;
        }
        let supply = token.accounting().total_supply()?;
        ...
        token.accounting_mut().emit_event(
            IH20::SupplyCapUpdated { updater: caller, oldSupplyCap: old, newSupplyCap: new_cap }
                .encode_log_data(),
        )
    }
    ...
    fn update_name(
        ...
        token
            .accounting_mut()
            .emit_event(IH20::NameUpdated { updater: caller, newName: name }.encode_log_data())?;
        token.accounting_mut().emit_event(IH20::EIP712DomainChanged {}.encode_log_data())
    }
    ...
    fn update_symbol(
        ...
        token.accounting_mut().emit_event(
            IH20::SymbolUpdated { updater: caller, newSymbol: symbol }.encode_log_data(),
        )
    }
    ...
    fn update_contract_uri(
        ...
        token.accounting_mut().emit_event(IH20::ContractURIUpdated {}.encode_log_data())
    }
    ...
    fn grant_role(
        ...
        if role == H20TokenRole::DefaultAdmin.id() || !privileged {
    ...
    fn renounce_last_admin(&self, token: &mut H20AssetToken<S, A>, caller: Address) -> Result<()> {
        let admin_role = H20TokenRole::DefaultAdmin.id();
        ...
        token
            .accounting_mut()
            .emit_event(IH20::LastAdminRenounced { previousAdmin: caller }.encode_log_data())
    }
    ...
    fn set_role_admin(
        ...
        if !privileged {
            self.ensure_role_admin_mutations_available(token, caller)?;
            H20Guards::ensure_role(token, caller, previous_admin_role)?;
        }
        token.accounting_mut().set_role_admin(role, new_admin_role)?;
        token.accounting_mut().emit_event(
            IH20::RoleAdminChanged {
                role,
    ...
    fn update_multiplier(
        ...
        token.accounting_mut().emit_event(
            IH20Asset::MultiplierUpdated { multiplier: new_multiplier }.encode_log_data(),
        )
    }
    ...
    fn update_extra_metadata(
        ...
        token
            .accounting_mut()
            .emit_event(IH20Asset::ExtraMetadataUpdated { key, value }.encode_log_data())
    }
    ...
    fn begin_announce(
        ...
        token
            .accounting_mut()
            .emit_event(IH20Asset::Announcement { caller, id, description, uri }.encode_log_data())
    }

    fn end_announce(&self, token: &mut H20AssetToken<S, A>, id: String) -> Result<()> {
        token.accounting_mut().emit_event(IH20Asset::EndAnnouncement { id }.encode_log_data())
    }
    ...
    fn is_paused(
        ...
        H20PausableFeature::ensure_valid(feature)?;
        Ok((token.accounting().paused()? & H20PausableFeature::mask(feature)) != U256::ZERO)
    }

    fn paused_features(&self, token: &H20AssetToken<S, A>) -> Result<Vec<IH20::PausableFeature>> {
    ...
    fn policy_id(&self, token: &H20AssetToken<S, A>, policy_scope: B256) -> Result<u64> {
        Self::ensure_supported_policy_type(policy_scope)?;
        token.accounting().policy_id(policy_scope)
    }

    fn domain_separator(&self, token: &H20AssetToken<S, A>, chain_id: u64) -> Result<B256> {
    ...
    fn eip712_domain(&self, token: &H20AssetToken<S, A>, chain_id: u64) -> Result<Eip712Domain> {
        let name = token.accounting().name()?;
        Ok((
    ...
    fn scaled_balance_of(&self, token: &H20AssetToken<S, A>, account: Address) -> Result<U256> {
...
mod tests {
    ...
    use crate::{
        Asset, AssetAccounting, AssetV1, H20_MAX_SUPPLY_CAP, H20AssetStorage, H20AssetToken,
        H20PolicyType, H20TokenRole, IH20, IH20Asset, PackedPolicy, PermitArgs, PolicyAccounting,
    ...
    type Tok = H20AssetToken<FakeAccounting, FakePolicyAccounting>;
    ...
    fn transfer_moves_balance_and_emits_transfer() {
        ...
        assert_eq!(last_event_sig(&tok), IH20::Transfer::SIGNATURE_HASH);
    }
    ...
    fn transfer_reverts_when_paused() {
        let mut tok = token();
        fund(&mut tok, ALICE, U256::from(10u64));
        LOGIC.pause(&mut tok, ADMIN, vec![IH20::PausableFeature::TRANSFER], true).unwrap();
    ...
    fn approve_sets_allowance_and_emits() {
        ...
        assert_eq!(last_event_sig(&tok), IH20::Approval::SIGNATURE_HASH);
    }
    ...
    fn burn_requires_role_then_decreases_supply() {
        ...
        grant(&mut tok, H20TokenRole::Burn.id(), ALICE);
    ...
    fn pause_and_unpause_toggle_feature_bit() {
        let mut tok = token();
        LOGIC.pause(&mut tok, ADMIN, vec![IH20::PausableFeature::MINT], true).unwrap();
        assert!(LOGIC.is_paused(&tok, IH20::PausableFeature::MINT).unwrap());
        LOGIC.unpause(&mut tok, ADMIN, vec![IH20::PausableFeature::MINT], true).unwrap();
        assert!(!LOGIC.is_paused(&tok, IH20::PausableFeature::MINT).unwrap());
    }
    ...
    fn grant_role_privileged_grants_and_emits() {
        let mut tok = token();
        LOGIC.grant_role(&mut tok, ADMIN, H20TokenRole::Mint.id(), ALICE, true).unwrap();
        assert!(tok.accounting().has_role(H20TokenRole::Mint.id(), ALICE).unwrap());
        assert_eq!(last_event_sig(&tok), IH20::RoleGranted::SIGNATURE_HASH);
    }

    #[test]
    fn revoke_last_admin_is_rejected() {
        let mut tok = token();
        grant(&mut tok, H20TokenRole::DefaultAdmin.id(), ADMIN);
        let err = LOGIC
    ...
    fn grant_role_unchecked_bumps_admin_count() {
        let mut tok = token();
        LOGIC
            .grant_role_unchecked(&mut tok, H20TokenRole::DefaultAdmin.id(), ADMIN, TOKEN)
            .unwrap();
        assert!(tok.accounting().has_role(H20TokenRole::DefaultAdmin.id(), ADMIN).unwrap());
        assert_eq!(
            tok.accounting().role_member_count(H20TokenRole::DefaultAdmin.id()).unwrap(),
            U256::ONE
        );
    }
    ...
    fn update_policy_sets_new_id() {
        ...
        LOGIC.update_policy(&mut tok, ADMIN, H20PolicyType::TransferSender.id(), 7, true).unwrap();
        assert_eq!(tok.accounting().policy_id(H20PolicyType::TransferSender.id()).unwrap(), 7);
    }
    ...
    fn to_scaled_balance_two_to_one_multiplier() {
        let mut tok = token();
        tok.accounting_mut().set_multiplier(H20AssetStorage::WAD * U256::from(2u64)).unwrap();
    ...
    fn update_multiplier_persists_and_emits() {
        let mut tok = token();
        let new_multiplier = H20AssetStorage::WAD * U256::from(3u64);
        ...
        assert_eq!(last_event_sig(&tok), IH20Asset::MultiplierUpdated::SIGNATURE_HASH);
    }
    ...
    fn update_extra_metadata_roundtrip_and_clear() {
        let mut tok = token();
        grant(&mut tok, H20TokenRole::Metadata.id(), ADMIN);
        LOGIC
        ...
        assert_eq!(last_event_sig(&tok), IH20Asset::ExtraMetadataUpdated::SIGNATURE_HASH);
    ...
    fn begin_announce_marks_id_and_emits() {
        ...
        LOGIC
            ...
            .unwrap();
        assert!(LOGIC.is_announcement_id_used(&tok, &id).unwrap());
        assert_eq!(last_event_sig(&tok), IH20Asset::Announcement::SIGNATURE_HASH);
    }
    ...
    fn end_announce_emits_end_event() {
        ...
        assert_eq!(last_event_sig(&tok), IH20Asset::EndAnnouncement::SIGNATURE_HASH);
    }
