...
impl<S: AssetAccounting, A: PolicyAccounting> H20AssetToken<S, A> {
    ...
    pub fn route<O>(
        ...
        privileged: bool,
        observer: O,
    ) -> h20_precompile_storage::Result<Bytes>
    where
        O: PrecompileCallObserver,
    {
        ...
        // Fall through to inherited IH20 selectors.
        let call = decode_precompile_call!(calldata, IH20::IH20Calls);
        let label = call.as_label();

        observer.observe(label, || self.handle_h20_call(ctx, call, version, privileged))
    }
    ...
    fn handle_h20_call(
        ...
        let encoded: Bytes = match call {
            ...
            C::DEFAULT_ADMIN_ROLE(_) => H20TokenRole::DefaultAdmin.id().abi_encode().into(),
            C::MINT_ROLE(_) => H20TokenRole::Mint.id().abi_encode().into(),
            C::BURN_ROLE(_) => H20TokenRole::Burn.id().abi_encode().into(),
            C::BURN_BLOCKED_ROLE(_) => H20TokenRole::BurnBlocked.id().abi_encode().into(),
            C::PAUSE_ROLE(_) => H20TokenRole::Pause.id().abi_encode().into(),
            C::UNPAUSE_ROLE(_) => H20TokenRole::Unpause.id().abi_encode().into(),
            C::METADATA_ROLE(_) => H20TokenRole::Metadata.id().abi_encode().into(),

            // --- Policy type identifiers (invariant across versions) ---
            C::TRANSFER_SENDER_POLICY(_) => H20PolicyType::TransferSender.id().abi_encode().into(),
            C::TRANSFER_RECEIVER_POLICY(_) => {
                H20PolicyType::TransferReceiver.id().abi_encode().into()
            }
            C::TRANSFER_EXECUTOR_POLICY(_) => {
                H20PolicyType::TransferExecutor.id().abi_encode().into()
            }
            C::MINT_RECEIVER_POLICY(_) => H20PolicyType::MintReceiver.id().abi_encode().into(),
            ...
            C::eip712Domain(_) => {
                ...
                IH20::eip712DomainCall::abi_encode_returns(&IH20::eip712DomainReturn {
                    fields,
                    name,
                    version,
                    chainId: chain_id,
        ...
        O: PrecompileCallObserver,
    {
        ...
        let encoded: Bytes = match call {
            ...
            SC::WAD_PRECISION(_) => H20AssetStorage::WAD.abi_encode().into(),
            ...
            SC::batchMint(c) => {
                observer.record_batch_items(
                    &BerylAuxiliaryMetrics::h20("asset", "batchMint"),
                    c.recipients.len(),
                );
        ...
        O: PrecompileCallObserver,
    {
        ...
        observer.record_internal_calls(
            &BerylAuxiliaryMetrics::h20("asset", "announce"),
            internal_call_count,
            internal_call_bytes,
        );
...
mod tests {
    ...
    use h20_precompile_storage::{HashMapStorageProvider, Result, StorageCtx};
    ...
    const UPGRADE: H20Spec = H20Spec::Beryl;
    ...
    fn make_token() -> TestAssetToken {
        ...
        accounting.multiplier = H20AssetStorage::WAD; // 1:1 multiplier
    ...
    fn activate_h20_asset(storage: &mut HashMapStorageProvider) {
        storage.set_caller(ACTIVATION_ADMIN);
        StorageCtx::enter(storage, |ctx| {
            ActivationRegistryStorage::new(ctx)
                .activate(ActivationFeature::H20Asset.id(), ACTIVATION_ADMIN_CONFIG)
        })
        .unwrap();
    }
    ...
    fn storage_with_caller(caller: Address) -> HashMapStorageProvider {
        ...
        activate_h20_asset(&mut storage);
    ...
    fn batch_mint_calldata(recipients: Vec<Address>, amounts: Vec<U256>) -> Vec<u8> {
        IH20Asset::batchMintCall { recipients, amounts }.abi_encode()
    }
    ...
    fn dispatch_with_observer_records_asset_success() {
        ...
        assert_eq!(calls[0].0.precompile, "h20");
    ...
    fn dispatch_with_observer_records_asset_decode_failure() {
        let observer = RecordingObserver::default();
        let mut token = make_token();
        let calldata = IH20::balanceOfCall::SELECTOR;
        let mut storage = storage_with_caller(ALICE);
    ...
    fn batch_mint_increases_balances() {
        let mut token = make_token();
        token.accounting_mut().roles.insert((H20TokenRole::Mint.id(), ALICE), true);
    ...
    /// not be wrapped as [`IH20Asset::InternalCallFailed`]. A deliberately overflowing
    ...
    fn announce_inner_system_error_propagates_unchanged() {
        ...
        let inner_call = Bytes::from(
            IH20Asset::toScaledBalanceCall { rawBalance: U256::from(2u64) }.abi_encode(),
        );
        let calldata = IH20Asset::announceCall {
    ...
    /// [`IH20Asset::InternalCallFailed`], preserving the original calldata in the error field.
    #[test]
    fn announce_inner_ordinary_revert_wraps_as_internal_call_failed() {
        ...
        let inner_call = Bytes::from(IH20::mintCall { to: BOB, amount: U256::ONE }.abi_encode());
        let calldata = IH20Asset::announceCall {
