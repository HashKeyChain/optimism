...
impl IH20Asset::IH20AssetCalls {
    ...
    pub const fn as_label(&self) -> &'static str {
        match self {
            ...
            Self::WAD_PRECISION(_) => "precompile-h20-asset-WAD_PRECISION",
            Self::announce(_) => "precompile-h20-asset-announce",
            Self::isAnnouncementIdUsed(_) => "precompile-h20-asset-isAnnouncementIdUsed",
            Self::multiplier(_) => "precompile-h20-asset-multiplier",
            Self::toScaledBalance(_) => "precompile-h20-asset-toScaledBalance",
            Self::toRawBalance(_) => "precompile-h20-asset-toRawBalance",
            Self::scaledBalanceOf(_) => "precompile-h20-asset-scaledBalanceOf",
            Self::updateMultiplier(_) => "precompile-h20-asset-updateMultiplier",
            Self::batchMint(_) => "precompile-h20-asset-batchMint",
            Self::extraMetadata(_) => "precompile-h20-asset-extraMetadata",
            Self::updateExtraMetadata(_) => "precompile-h20-asset-updateExtraMetadata",
        }
    }
}
...
mod tests {
    ...
    use crate::{IH20, IH20Asset};

    #[test]
    fn asset_call_labels_are_stable() {
        assert_eq!(
            IH20Asset::IH20AssetCalls::updateExtraMetadata(IH20Asset::updateExtraMetadataCall {
                key: alloc::string::String::new(),
                value: alloc::string::String::new(),
            })
            .as_label(),
            "precompile-h20-asset-updateExtraMetadata"
        );
    }

    #[test]
    fn asset_and_inherited_call_selectors_are_disjoint() {
        for selector in IH20Asset::IH20AssetCalls::selectors() {
            assert!(
                !IH20::IH20Calls::valid_selector(selector),
                "asset selector {selector:?} overlaps with inherited IH20 selector"
            );
        }

        for selector in IH20::IH20Calls::selectors() {
            assert!(
                !IH20Asset::IH20AssetCalls::valid_selector(selector),
                "inherited IH20 selector {selector:?} overlaps with asset selector"
            );
        }
    }
}
