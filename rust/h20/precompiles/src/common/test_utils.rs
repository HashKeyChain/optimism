...
pub type TestStablecoinToken = H20StablecoinToken<InMemoryTokenAccounting, FakePolicyAccounting>;

/// Concrete test token that opts into the shared capability traits over the in-memory fakes.
///
/// The production holders ([`crate::H20AssetToken`], [`crate::H20StablecoinToken`]) are now minimal
...
pub struct InMemoryTokenAccounting {
    ...
    /// Defaults to [`H20_MAX_SUPPLY_CAP`] so mint tests don't need to set a cap explicitly.
    pub supply_cap: U256,
    /// Token name.
    pub name: String,
    /// Token symbol.
...
impl AssetAccounting for InMemoryTokenAccounting {
    fn multiplier(&self) -> Result<U256> {
        Ok(if self.multiplier.is_zero() { H20AssetStorage::WAD } else { self.multiplier })
    }
    ...
    fn decimals(&self) -> Result<u8> {
        Ok(if self.decimals == 0 { H20AssetStorage::MIN_DECIMALS } else { self.decimals })
    }
}
