...
pub trait Configurable: Token {
    ...
    fn update_supply_cap(
        ...
        if !privileged {
            H20Guards::ensure_token_role::<Self>(self, caller, H20TokenRole::DefaultAdmin)?;
        }
    ...
    fn update_name(&mut self, caller: Address, name: String, privileged: bool) -> Result<()> {
        if !privileged {
            H20Guards::ensure_token_role::<Self>(self, caller, H20TokenRole::Metadata)?;
        }
        self.accounting_mut().set_name(name.clone())?;
        self.accounting_mut()
            .emit_event(IH20::NameUpdated { updater: caller, newName: name }.encode_log_data())?;
        self.accounting_mut().emit_event(IH20::EIP712DomainChanged {}.encode_log_data())
    }
    ...
    fn update_contract_uri(
        ...
        self.accounting_mut().emit_event(IH20::ContractURIUpdated {}.encode_log_data())
    }
}
...
mod tests {
    ...
    fn token_with_default_admin(account: Address) -> TestToken {
        ...
        accounting.roles.insert((H20TokenRole::DefaultAdmin.id(), account), true);
    ...
    fn update_supply_cap_above_max_supply_cap_reverts() {
        let mut token = make_token();
        let proposed_cap = H20_MAX_SUPPLY_CAP + U256::ONE;
    ...
    fn non_privileged_config_update_with_admin_role_succeeds() {
        ...
        token.accounting_mut().roles.insert((H20TokenRole::Metadata.id(), CALLER), true);
    ...
    fn token_with_metadata_role(account: Address) -> TestToken {
        let mut accounting = InMemoryTokenAccounting::new(TOKEN_ADDR);
        accounting.roles.insert((H20TokenRole::Metadata.id(), account), true);
        TestToken::with_storage_and_policy(accounting, FakePolicyAccounting::new())
    }
