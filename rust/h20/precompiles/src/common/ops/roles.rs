...
pub trait RoleManaged: Token {
    ...
    /// Role required for `burn` and `burnWithMemo`.
    fn burn_role() -> B256 {
        H20TokenRole::Burn.id()
    }

    /// Role required for `burnBlocked`; permits burning from blocked accounts without `BURN_ROLE`.
    fn burn_blocked_role() -> B256 {
        H20TokenRole::BurnBlocked.id()
    }

    /// Role required for `pause`.
    fn pause_role() -> B256 {
        H20TokenRole::Pause.id()
    }

    /// Role required for `unpause`.
    fn unpause_role() -> B256 {
        H20TokenRole::Unpause.id()
    }
    ...
    fn ensure_role(&self, caller: Address, role: B256) -> Result<()> {
        H20Guards::ensure_role(self, caller, role)
    }
...
mod tests {
    ...
    fn token_with_default_admin() -> TestToken {
        ...
        accounting.roles.insert((H20TokenRole::DefaultAdmin.id(), ADMIN), true);
        accounting.role_member_counts.insert(H20TokenRole::DefaultAdmin.id(), U256::ONE);
    ...
    fn grant_role_authorizes_against_role_admin_and_emits_event() {
        ...
        token.grant_role(ADMIN, H20TokenRole::Mint.id(), ALICE, false).unwrap();

        assert!(token.has_role(H20TokenRole::Mint.id(), ALICE).unwrap());
        assert_eq!(
            token.accounting().events[0],
            IH20::RoleGranted { role: H20TokenRole::Mint.id(), account: ALICE, sender: ADMIN }
                .encode_log_data()
        );
    }
    ...
    fn revoke_role_rejects_final_default_admin() {
        ...
        assert_eq!(
            token.accounting().role_member_count(H20TokenRole::DefaultAdmin.id()).unwrap(),
            U256::ONE
        );
    }
    ...
    fn grant_role_rejects_default_admin_after_renounce_last_admin_even_privileged() {
        ...
        assert!(!token.has_role(H20TokenRole::DefaultAdmin.id(), ALICE).unwrap());
        assert_eq!(
            token.accounting().role_member_count(H20TokenRole::DefaultAdmin.id()).unwrap(),
            U256::ZERO
        );
    ...
    fn grant_role_allows_non_admin_role_after_renounce_last_admin_privileged() {
        ...
        token.grant_role(ALICE, H20TokenRole::Mint.id(), ALICE, true).unwrap();
    ...
    fn revoke_role_allows_non_final_default_admin() {
        let mut token = token_with_default_admin();
        token.grant_role(ADMIN, H20TokenRole::DefaultAdmin.id(), ALICE, false).unwrap();

        token.revoke_role(ADMIN, H20TokenRole::DefaultAdmin.id(), ALICE, false).unwrap();
    ...
    fn renounce_last_admin_revokes_and_emits_terminal_event() {
        ...
        assert_eq!(
            ...
            &IH20::LastAdminRenounced { previousAdmin: ADMIN }.encode_log_data()
        );
    }
    ...
    fn renounce_last_admin_prevents_custom_admin_resurrection() {
        ...
        token.set_role_admin(ADMIN, H20TokenRole::DefaultAdmin.id(), CUSTOM_ROLE, false).unwrap();
    ...
    fn renounce_last_admin_disables_role_admin_mutations() {
        ...
        token.set_role_admin(ADMIN, H20TokenRole::Mint.id(), CUSTOM_ROLE, false).unwrap();
        token.grant_role(ADMIN, CUSTOM_ROLE, ALICE, false).unwrap();
        token.renounce_last_admin(ADMIN).unwrap();
    ...
    fn renounce_last_admin_disables_custom_admin_revoke() {
        ...
        token.set_role_admin(ADMIN, H20TokenRole::Mint.id(), CUSTOM_ROLE, false).unwrap();
        token.grant_role(ADMIN, CUSTOM_ROLE, ALICE, false).unwrap();
        token.grant_role(ALICE, H20TokenRole::Mint.id(), BOB, false).unwrap();
        token.renounce_last_admin(ADMIN).unwrap();
        ...
        assert!(token.has_role(H20TokenRole::Mint.id(), BOB).unwrap());
    }
    ...
    fn renounce_last_admin_disables_custom_admin_reassignment() {
        ...
        assert_eq!(token.role_admin(H20TokenRole::Mint.id()).unwrap(), CUSTOM_ROLE);
    }
    ...
    fn set_role_admin_emits_previous_and_new_admin_roles() {
        ...
        token.set_role_admin(ADMIN, CUSTOM_ROLE, H20TokenRole::Mint.id(), false).unwrap();

        assert_eq!(token.role_admin(CUSTOM_ROLE).unwrap(), H20TokenRole::Mint.id());
        assert_eq!(
            token.accounting().events[0],
            IH20::RoleAdminChanged {
                role: CUSTOM_ROLE,
                previousAdminRole: B256::ZERO,
                newAdminRole: H20TokenRole::Mint.id(),
            }
            .encode_log_data()
        );
    }
