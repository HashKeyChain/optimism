...
pub trait Mintable: Token {
    ...
    fn mint(&mut self, caller: Address, to: Address, amount: U256, privileged: bool) -> Result<()> {
        H20Guards::ensure_not_paused::<Self>(self, IH20::PausableFeature::MINT)?;
        if !privileged {
...
mod tests {
    ...
    fn non_privileged_mint_with_role_succeeds() {
        let mut token = token_with_role(H20TokenRole::Mint, CALLER);
    ...
    fn mint_reverts_when_mint_feature_paused() {
        let mut accounting = InMemoryTokenAccounting::new(TOKEN_ADDR);
        accounting.paused = H20PausableFeature::mask(IH20::PausableFeature::MINT);
    ...
    fn mint_paused_gets_pause_error_not_policy_error() {
        ...
        accounting
            .policy_ids
            .insert(H20PolicyType::MintReceiver.id(), PolicyRegistryStorage::ALWAYS_BLOCK_ID);
        let mut token = TestToken::with_storage_and_policy(accounting, FakePolicyAccounting::new());
    ...
    fn mint_guard_ordering(
        ...
        if has_role {
            accounting.roles.insert((H20TokenRole::Mint.id(), CALLER), true);
        }
        if policy_blocks {
            accounting
                .policy_ids
