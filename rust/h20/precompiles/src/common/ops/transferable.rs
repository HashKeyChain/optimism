...
pub trait Transferable: Token {
    ...
    fn transfer_inner(
        ...
        if !privileged {
            ...
            H20Guards::ensure_policy_type::<Self>(self, H20PolicyType::TransferReceiver, to)?;
        }
    ...
    fn transfer_from(
        ...
        if !privileged && spender != from {
            H20Guards::ensure_policy_type::<Self>(self, H20PolicyType::TransferExecutor, spender)?;
        }
        self.transfer_inner(from, to, amount, privileged)?;
        if is_infinite {
            return Ok(());
    ...
    fn approve(&mut self, owner: Address, spender: Address, amount: U256) -> Result<()> {
        ...
        self.accounting_mut()
            .emit_event(IH20::Approval { owner, spender, amount }.encode_log_data())
    }
    ...
    fn transfer_with_memo(
        ...
        self.accounting_mut().emit_event(IH20::Memo { caller: from, memo }.encode_log_data())
    }
    ...
    fn transfer_from_with_memo(
        ...
        self.accounting_mut().emit_event(IH20::Memo { caller: spender, memo }.encode_log_data())
    }
}
...
mod tests {
    ...
    fn transfer_to_self_preserves_balance_and_emits_event() {
        ...
        assert_eq!(
            ...
            IH20::Transfer { from: ALICE, to: ALICE, amount: U256::from(30u64) }.encode_log_data()
        );
    }
    ...
    fn transfer_reverts_when_receiver_policy_denies() {
        ...
        accounting
            .policy_ids
            .insert(H20PolicyType::TransferReceiver.id(), PolicyRegistryStorage::ALWAYS_BLOCK_ID);
        let mut token = TestToken::with_storage_and_policy(accounting, FakePolicyAccounting::new());
    ...
    fn transfer_from_reverts_when_executor_policy_denies() {
        ...
        accounting
            .policy_ids
            .insert(H20PolicyType::TransferExecutor.id(), PolicyRegistryStorage::ALWAYS_BLOCK_ID);
        let mut token = TestToken::with_storage_and_policy(accounting, FakePolicyAccounting::new());
    ...
    fn transfer_allowed_by_external_sender_policy_succeeds() {
        ...
        let mut accounting = InMemoryTokenAccounting::new(TOKEN_ADDR);
        accounting.balances.insert(ALICE, U256::from(10u64));
        accounting.policy_ids.insert(H20PolicyType::TransferSender.id(), POLICY_ID);
    ...
    fn transfer_allowed_by_external_receiver_policy_succeeds() {
        ...
        let mut accounting = InMemoryTokenAccounting::new(TOKEN_ADDR);
        accounting.balances.insert(ALICE, U256::from(10u64));
        accounting.policy_ids.insert(H20PolicyType::TransferReceiver.id(), POLICY_ID);
        let mut policy = FakePolicyAccounting::new();
    ...
    fn transfer_emits_transfer_event_with_correct_fields() {
        ...
        assert_eq!(token.accounting().events.len(), 1);
        let decoded = IH20::Transfer::decode_log_data(&token.accounting().events[0]).unwrap();
    ...
    fn transfer_from_with_memo_emits_transfer_then_memo() {
        ...
        assert_eq!(token.accounting().events.len(), 2);
        IH20::Transfer::decode_log_data(&token.accounting().events[0]).unwrap();
    }
    ...
    fn transfer_from_guard_ordering(
        ...
        if executor_blocked {
            accounting.policy_ids.insert(
                H20PolicyType::TransferExecutor.id(),
