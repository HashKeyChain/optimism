//! ABI dispatch for the `H20Factory` precompile.
//!
...
//! `createH20`'s business logic to it. `getH20Address`, `isH20`, and
//! `isH20Initialized` are answered via version-invariant computations/pass-throughs.
...
impl<'a> H20FactoryStorage<'a> {
    /// ABI-dispatches `calldata` to the appropriate `IH20Factory` handler for `upgrade`.
    pub fn dispatch(
        &mut self,
        ctx: StorageCtx<'_>,
        calldata: &[u8],
    ...
    /// ABI-dispatches `calldata` to the appropriate `IH20Factory` handler with an observer.
    ...
    pub fn create_h20(
        ...
        FactoryV1.create_h20(self, call, address_hash, upgrade)
    }
        ...
        O: PrecompileCallObserver,
    {
        ...
        match decode_precompile_call!(calldata, IH20Factory::IH20FactoryCalls) {
            IH20Factory::IH20FactoryCalls::createH20(call) => {
                ...
                let variant = H20Variant::from_abi(call.variant).expect(
                    "abi_decode_validate rejects non-canonical discriminants before dispatch",
                );
                ...
                let token = logic.create_h20(self, call, address_hash, upgrade)?;
                observer.record_internal_calls(
                    &BerylAuxiliaryMetrics::singleton("factory", "createH20"),
                    internal_call_count,
                    internal_call_bytes,
                );
                observer.record_h20_created(variant.as_label());
                Ok(IH20Factory::createH20Call::abi_encode_returns(&token).into())
            }
            IH20Factory::IH20FactoryCalls::getH20Address(call) => {
                let v = H20Variant::from_abi(call.variant).expect(
                    "abi_decode_validate rejects non-canonical discriminants before dispatch",
                );
                let hash = ctx.metered_keccak256(&(call.sender, call.salt).abi_encode())?;
                let addr = v.compute_address_from_hash(hash).0;
                Ok(IH20Factory::getH20AddressCall::abi_encode_returns(&addr).into())
            }
            IH20Factory::IH20FactoryCalls::isH20(call) => {
                let result = logic.is_h20(self, call.token)?;
                Ok(IH20Factory::isH20Call::abi_encode_returns(&result).into())
            }
            IH20Factory::IH20FactoryCalls::isH20Initialized(call) => {
                let initialized = logic.is_h20_initialized(self, call.token)?;
                Ok(IH20Factory::isH20InitializedCall::abi_encode_returns(&initialized).into())
            }
        }
    }
}
...
mod tests {
    ...
    use crate::{
        ...
        H20AssetStorage, H20AssetToken, H20FactoryStorage, H20StablecoinStorage, H20Variant, IH20,
        IH20Factory, PolicyRegistryStorage, PolicyVersion,
    };
    ...
    fn dispatch_factory_success(ctx: StorageCtx<'_>, call: impl SolCall) -> Bytes {
        ...
        let output = factory.dispatch(ctx, &call.abi_encode(), H20Spec::Beryl).unwrap();
    ...
    fn dispatch_h20_success(ctx: StorageCtx<'_>, token_addr: Address, call: impl SolCall) -> Bytes {
        let mut token = token_at(token_addr, ctx);
        let output = token.dispatch(ctx, &call.abi_encode(), H20Spec::Beryl).unwrap();
    ...
    fn dispatch_rejects_call_with_nonzero_value() {
        ...
        let calldata = IH20Factory::isH20Call { token: Address::ZERO }.abi_encode();

        let out = StorageCtx::enter(&mut storage, |ctx| {
            H20FactoryStorage::new(ctx).dispatch(ctx, &calldata, H20Spec::Beryl)
        })
        ...
        assert_eq!(
            out.bytes,
            alloy_primitives::Bytes::from(IH20Factory::NonPayable {}.abi_encode())
        );
    }
    ...
    fn test_create_default_token_checks_version() {
        ...
        params.version = H20Variant::Asset.supported_version() + 1;
        let call = create_call(IH20Factory::H20Variant::ASSET, params, B256::repeat_byte(0x55));

        StorageCtx::enter(&mut storage, |ctx| {
            assert_output(
                dispatch_factory_revert(ctx, call),
                IH20Factory::UnsupportedVersion {
                    version: H20Variant::Asset.supported_version() + 1,
    ...
    fn test_create_token_reverts_for_invalid_params_encoding() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            assert!(output.starts_with(&IH20Factory::createH20Call::SELECTOR));
        });
    }
    ...
    fn test_create_token_reverts_for_missing_stablecoin_currency() {
        let mut storage = HashMapStorageProvider::new(1);
        activate_precompiles(&mut storage);
        let params = IH20Factory::H20StablecoinCreateParams {
            version: H20Variant::Stablecoin.supported_version(),
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            assert_output(
                dispatch_factory_revert(ctx, call),
                IH20Factory::MissingRequiredField { field: "currency".to_string() }.abi_encode(),
            );
        });
    }
    ...
    fn test_create_token_reverts_for_invalid_stablecoin_currency_format() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            assert_output(
                dispatch_factory_revert(ctx, call),
                IH20Factory::InvalidCurrency { code: "usd".to_string() }.abi_encode(),
            );
        });
    }
    ...
    fn test_create_token_checks_stablecoin_version_before_currency() {
        ...
        activate_precompiles(&mut storage);
        let params = IH20Factory::H20StablecoinCreateParams {
            version: H20Variant::Stablecoin.supported_version() + 1,
            name: "Stablecoin Token".to_string(),
            symbol: "USD".to_string(),
            initialAdmin: Address::repeat_byte(0xAB),
            currency: String::new(),
    ...
    fn test_create_token_supports_stablecoin() {
        ...
        let stablecoin_call = IH20Factory::createH20Call {
            variant: IH20Factory::H20Variant::STABLECOIN,
            salt: B256::repeat_byte(0x08),
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            let stablecoin_addr = IH20Factory::createH20Call::abi_decode_returns(
                dispatch_factory_success(ctx, stablecoin_call).as_ref(),
            )
            .unwrap();
            let stablecoin = H20StablecoinStorage::from_address(stablecoin_addr, ctx);
            assert_eq!(stablecoin.stablecoin.currency.read().unwrap(), "USD");
            assert_eq!(stablecoin.h20.name.read().unwrap(), "Stablecoin Token");
            assert_eq!(H20Variant::from_address(stablecoin_addr), Some(H20Variant::Stablecoin));
        });
    }
    ...
    fn test_create_asset_token_stores_decimals_and_multiplier() {
        ...
        let asset_call = IH20Factory::createH20Call {
            variant: IH20Factory::H20Variant::ASSET,
            salt,
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            assert_output(
                ...
                IH20Factory::createH20Call::abi_encode_returns(&expected_addr),
            );
            assert!(ctx.has_bytecode(expected_addr).unwrap());

            let asset_storage = H20AssetStorage::from_address(expected_addr, ctx);
            assert_eq!(asset_storage.h20.name.read().unwrap(), "Asset Token");
            assert_eq!(asset_storage.h20.symbol.read().unwrap(), "AST");
    ...
    fn test_factory_dispatch_create_token_predicts_and_initializes_token() {
        ...
        let (expected_token, _) = H20Variant::Asset.compute_address(creator, salt);
        let mut call = create_call(
            IH20Factory::H20Variant::ASSET,
            token_params("Dispatch Token", "DSP"),
            salt,
        );
        call.initCalls.push(
            IH20::mintCall { to: Address::repeat_byte(0xCD), amount: U256::from(1_000u64) }
                .abi_encode()
                .into(),
        );
        call.initCalls.push(
            IH20::updateContractURICall { newURI: "ipfs://dispatch".to_string() }
                .abi_encode()
                .into(),
        );
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            assert_output(
                dispatch_factory_success(
                    ctx,
                    IH20Factory::getH20AddressCall {
                        variant: IH20Factory::H20Variant::ASSET,
                        sender: creator,
                        salt,
                    },
                ),
                IH20Factory::getH20AddressCall::abi_encode_returns(&expected_token),
            );
            assert_output(
                dispatch_factory_success(ctx, call),
                IH20Factory::createH20Call::abi_encode_returns(&expected_token),
            );
            assert!(ctx.has_bytecode(expected_token).unwrap());

            assert_output(
                dispatch_factory_success(ctx, IH20Factory::isH20Call { token: expected_token }),
                IH20Factory::isH20Call::abi_encode_returns(&true),
            );

            assert_output(
                dispatch_h20_success(ctx, expected_token, IH20::nameCall {}),
                "Dispatch Token".to_string().abi_encode(),
            );
            assert_output(
                dispatch_h20_success(ctx, expected_token, IH20::symbolCall {}),
                "DSP".to_string().abi_encode(),
            );
            assert_output(
                dispatch_h20_success(ctx, expected_token, IH20::decimalsCall {}),
                IH20::decimalsCall::abi_encode_returns(&6u8),
            );
            assert_output(
                dispatch_h20_success(ctx, expected_token, IH20::totalSupplyCall {}),
                U256::from(1_000u64).abi_encode(),
            );
            assert_output(
                dispatch_h20_success(
                    ctx,
                    expected_token,
                    IH20::balanceOfCall { account: Address::repeat_byte(0xCD) },
                ),
                U256::from(1_000u64).abi_encode(),
            );
            assert_output(
                dispatch_h20_success(ctx, expected_token, IH20::contractURICall {}),
    ...
    fn test_uninitialized_prefix_token_reverts() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            let (token_addr, tail) =
                H20Variant::Asset.compute_address(caller, B256::repeat_byte(0x09));
            assert_eq!(token_addr.as_slice()[11..], tail);
            assert!(!ctx.has_bytecode(token_addr).unwrap());

            let mut token = token_at(token_addr, ctx);
            let result =
                token.dispatch(ctx, &IH20::nameCall {}.abi_encode(), H20Spec::Beryl).unwrap();
    ...
    #[test]
    fn test_h20_dispatch_transfer_approve_transfer_from() {
        let creator = Address::repeat_byte(0xCA);
        ...
        let (token_addr, _) = H20Variant::Asset.compute_address(creator, salt);
        let mut call = create_call(
            IH20Factory::H20Variant::ASSET,
            token_params("Dispatch Token", "DSP"),
            salt,
        ...
        call.initCalls
            .push(IH20::mintCall { to: alice, amount: U256::from(1_000u64) }.abi_encode().into());
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            assert_output(
                dispatch_factory_success(ctx, call),
                IH20Factory::createH20Call::abi_encode_returns(&token_addr),
            );
        });
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            assert_output(
                dispatch_h20_success(
                    ctx,
                    token_addr,
                    IH20::transferCall { to: bob, amount: U256::from(300u64) },
                ),
                true.abi_encode(),
            );
            assert_output(
                dispatch_h20_success(
                    ctx,
                    token_addr,
                    IH20::approveCall { spender, amount: U256::from(250u64) },
                ),
                true.abi_encode(),
            );
        });
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            assert_output(
                dispatch_h20_success(
                    ctx,
                    token_addr,
                    IH20::transferFromCall { from: alice, to: charlie, amount: U256::from(200u64) },
                ),
                true.abi_encode(),
            );
            assert_output(
                dispatch_h20_success(ctx, token_addr, IH20::balanceOfCall { account: alice }),
                U256::from(500u64).abi_encode(),
            );
            assert_output(
                dispatch_h20_success(ctx, token_addr, IH20::balanceOfCall { account: bob }),
                U256::from(300u64).abi_encode(),
            );
            assert_output(
                dispatch_h20_success(ctx, token_addr, IH20::balanceOfCall { account: charlie }),
            ...
            assert_output(
                dispatch_h20_success(
                    ctx,
                    token_addr,
                    IH20::allowanceCall { owner: alice, spender },
                ),
    ...
    #[test]
    fn h20created_asset_variant_emits_empty_variant_params() {
        let mut storage = HashMapStorageProvider::new(1);
        activate_precompiles(&mut storage);
        let call = IH20Factory::createH20Call {
            variant: IH20Factory::H20Variant::ASSET,
        ...
        let event = storage
            .get_events(H20FactoryStorage::ADDRESS)
            .iter()
            .find_map(|l| IH20Factory::H20Created::decode_log_data(l).ok())
            .expect("H20Created must be emitted");
        assert!(event.variantParams.is_empty(), "ASSET variantParams must be empty");
    }

    #[test]
    fn h20created_stablecoin_variant_emits_encoded_currency() {
        let mut storage = HashMapStorageProvider::new(1);
        activate_precompiles(&mut storage);
        let call = IH20Factory::createH20Call {
            variant: IH20Factory::H20Variant::STABLECOIN,
            salt: B256::repeat_byte(0x71),
            params: IH20Factory::H20StablecoinCreateParams {
                version: 1,
        ...
        let params = IH20Factory::H20StablecoinEventParams::abi_decode(&event.variantParams)
            .expect("variantParams must decode as H20StablecoinEventParams");
        // Version byte frozen by `FactoryV1` for `H20StablecoinEventParams`.
        assert_eq!(params.version, 1);
        assert_eq!(params.currency, "USD");
    }

    #[test]
    fn get_h20_address_reverts_for_invalid_variant() {
        let mut storage = HashMapStorageProvider::new(1);
        activate_precompiles(&mut storage);
    ...
    #[test]
    fn h20created_asset_event_emits_token_specific_decimals() {
        // Regression: H20Created.decimals for an asset token must reflect init.decimals
        ...
        assert_eq!(
            event.decimals, 12,
            "H20Created.decimals must equal init.decimals, not any variant constant"
        );
    }
    ...
    fn factory_address_hashing_is_metered_for_valid_variant() {
        ...
        let (expected_asset_addr, _) = H20Variant::Asset.compute_address(sender, salt);
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            assert_output(
                ...
                IH20Factory::getH20AddressCall::abi_encode_returns(&expected_asset_addr),
            );
        });
        // One keccak call for the valid getH20Address.
        assert_eq!(
            storage.counter_keccak256(),
            1,
            "getH20Address must call keccak256 exactly once for a valid variant"
        );

        // createH20 also meters the keccak hash for valid variants. Verify the token
        // is created at the same address that getH20Address predicted.
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            ...
            assert_output(
                dispatch_factory_success(ctx, call),
                IH20Factory::createH20Call::abi_encode_returns(&expected_asset_addr),
            );
        });
        assert_eq!(
            storage.counter_keccak256(),
            1,
            "createH20 must call keccak256 exactly once for a valid variant"
        );
    }
}
