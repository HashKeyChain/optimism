...
use h20_precompile_storage::{Handler, Mapping, StorageCtx, StorageKey, setup_storage};
...
mod mapping_only_storable_layout {
    ...
    use h20_precompile_storage::{Mapping, StorableType};
...
mod namespaced_layout {
    ...
    use h20_precompile_macros::{Storable, contract};
    use h20_precompile_storage::{Handler, Mapping, StorageCtx, StorageKey, setup_storage};
...
mod type_namespaced_layouts {
    ...
    pub struct H20AssetLayout {
        pub local_head: u8,
        pub h20: H20Storage,
        pub asset: H20AssetStorage,
        pub redeem: H20RedeemStorage,
        pub local_tail: u16,
    }

    #[test]
    fn type_level_namespaces_mount_layouts_without_repeating_strings() {
        let h20_value = H20Storage { total_supply: U256::ZERO, balances: Mapping::default() };
        let asset_value = H20AssetStorage {
            multiplier: U256::ZERO,
            used_announcement_ids: Mapping::default(),
            extra_metadata: Mapping::default(),
        };
        let redeem_value =
            H20RedeemStorage { minimum_redeemable: U256::ZERO, redeem_policy_ids: U256::ZERO };
        let _ = (
            &h20_value.total_supply,
            &h20_value.balances,
        ...
        let h20_root = erc7201_root("h20");
        let asset_root = erc7201_root("h20.asset");
        let redeem_root = erc7201_root("h20.redeem");

        assert_eq!(<H20Storage as StorableType>::STORAGE_NAMESPACE_ID, "h20");
        assert_eq!(<H20Storage as StorableType>::STORAGE_NAMESPACE_ROOT, h20_root);
        assert_eq!(<H20AssetStorage as StorableType>::STORAGE_NAMESPACE_ROOT, asset_root);
        assert_eq!(<H20RedeemStorage as StorableType>::STORAGE_NAMESPACE_ROOT, redeem_root);
    ...
    fn type_level_namespaced_layouts_round_trip_through_handlers() {
        ...
        StorageCtx::enter(&mut storage, |ctx| {
            let mut layout = H20AssetLayout::new(ctx);

            layout.local_head.write(0x11).unwrap();
            layout.h20.total_supply.write(U256::from(100)).unwrap();
            layout.h20.balances.at_mut(&holder).write(U256::from(25)).unwrap();
            ...
            assert_eq!(layout.h20.total_supply.read().unwrap(), U256::from(100));
            assert_eq!(layout.h20.balances.at(&holder).read().unwrap(), U256::from(25));
            ...
            assert_eq!(
                ctx.sload(
                    TYPE_NAMESPACE_ADDR,
                    slots::H20 + U256::from(__packing_h20_storage::TOTAL_SUPPLY_LOC.offset_slots),
                )
                .unwrap(),
                U256::from(100)
            );
            assert_eq!(
                ctx.sload(
                    TYPE_NAMESPACE_ADDR,
                    holder.mapping_slot(
                        slots::H20 + U256::from(__packing_h20_storage::BALANCES_LOC.offset_slots),
                    ),
                )
                .unwrap(),
                U256::from(25)
...
mod struct_level_attribute_passthrough {
    ...
    mod allow_and_doc {
        use alloy_primitives::{Address, U256, address};
        use h20_precompile_macros::contract;
        use h20_precompile_storage::{Handler, StorageCtx, setup_storage};
