...
pub(crate) fn gen_handler_field_decl(field: &LayoutField<'_>) -> proc_macro2::TokenStream {
    ...
    let handler_type = match &field.kind {
        FieldKind::Direct(ty) => {
            quote! { <#ty as ::h20_precompile_storage::StorableType>::Handler<'a> }
        }
        FieldKind::Mapping { key, value } => {
            quote! { <::h20_precompile_storage::Mapping<#key, #value> as ::h20_precompile_storage::StorableType>::Handler<'a> }
        }
    };
...
pub(crate) fn gen_handler_field_init(
    ...
    match &field.kind {
        FieldKind::Direct(ty) => {
            ...
            quote! {
                #field_name: <#ty as ::h20_precompile_storage::StorableType>::handle(
        ...
        FieldKind::Mapping { key, value } => {
            quote! {
                #field_name: <::h20_precompile_storage::Mapping<#key, #value> as ::h20_precompile_storage::StorableType>::handle(
                    #slot_expr, ::h20_precompile_storage::LayoutCtx::FULL, address, storage
                )
            }
        }
    }
...
pub(crate) fn gen_constructor(
    ...
    let new_fn = address.map(|addr| {
        quote! {
            ...
            pub fn new(storage: ::h20_precompile_storage::StorageCtx<'a>) -> Self {
    ...
    quote! {
        impl<'a> #name<'a> {
            ...
            #[inline(always)]
            fn __initialize(&mut self) -> ::h20_precompile_storage::Result<()> {
            ...
            #[inline(always)]
            fn emit_event(&mut self, event: impl ::alloy_primitives::IntoLogData) -> ::h20_precompile_storage::Result<()> {
...
pub(crate) fn gen_contract_storage_impl(name: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl<'a> ::h20_precompile_storage::ContractStorage<'a> for #name<'a> {
            #[inline(always)]
            ...
            #[inline(always)]
            fn storage(&self) -> ::h20_precompile_storage::StorageCtx<'a> {
                self.storage
            }
        }
    }
